...
Note: The rule "do not add `<header>` to presentation pages" in `jssp-accessibility.instructions.md` is intended **to avoid duplication with the platform's global `<header>`**, and the imds `<header class="imds-header">` (in-page header) is exempt from this rule.
