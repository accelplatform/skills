...
**本步骤必须通过 `.github/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js` 的 `reflector.reflect()`（内部调用 `reflectProcessIdReplacements()`）执行。
...
反映逻辑实现于 `.github/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js`。以下为其概要与调用方法。
