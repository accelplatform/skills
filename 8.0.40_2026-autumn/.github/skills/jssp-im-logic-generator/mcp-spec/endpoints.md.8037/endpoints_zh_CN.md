...
工具名遵循 `mcp__im_logic__<操作名>` 格式（snake_case）。
...
参见 [mcp__im_logic__list_task_types.response.json](schemas/mcp__im_logic__list_task_types.response.json)。
...
- `hasEntityId: true` 的任务在制作 spec 时需要追加调用 `mcp__im_logic__resolve_entity_schema`（用户定义任务始终为 `false`）
...
参见 [mcp__im_logic__get_task_template.response.json](schemas/mcp__im_logic__get_task_template.response.json)。
...
- 对于实体任务（`hasEntityId: true`），`dataMapMetadata.inputDataDefinition` / `outputDataDefinition` 返回未指定实体时的默认值（空或通用 schema）。基于实际实体的类型应通过 `mcp__im_logic__resolve_entity_schema` 取得并覆盖
...
- 用户定义任务的 `inputDataDefinition` / `outputDataDefinition` 由用户在租户侧自由定义，无需通过 `mcp__im_logic__resolve_entity_schema` 覆盖
...
**由于向 `mcp__im_logic__resolve_entity_schema` 传入不存在的 `entityId` 可能导致挂起，请先用本端点定位真实存在的 `entityId` 再传入。**
...
参见 [mcp__im_logic__list_entities.response.json](schemas/mcp__im_logic__list_entities.response.json)。
...
- 不含字段（列）定义的轻量概要。字段定义通过 `mcp__im_logic__resolve_entity_schema` 取得（与 `mcp__im_logic__list_task_types` ⇒ `mcp__im_logic__get_task_template` 相同的两段式）
...
参见 [mcp__im_logic__resolve_entity_schema.response.json](schemas/mcp__im_logic__resolve_entity_schema.response.json)。
...
- 返回的 `inputDataDefinition` / `outputDataDefinition` 用于覆盖通过 `mcp__im_logic__get_task_template` 取得的模板的 `dataMapMetadata` 内的同名字段
...
参见 [mcp__im_logic__list_mapping_functions.response.json](schemas/mcp__im_logic__list_mapping_functions.response.json)。
...
- `argCount` 可大致了解参数数量。详情通过 `mcp__im_logic__get_mapping_function` 取得
...
参见 [mcp__im_logic__get_mapping_function.response.json](schemas/mcp__im_logic__get_mapping_function.response.json)。
...
2. `mcp__im_logic__list_entities(query?, category?)` ⇒ `mcp__im_logic__resolve_entity_schema(keyId, entityId)`  ※ 仅在使用实体时
   - 用 `mcp__im_logic__list_entities` 定位真实存在的 `entityId`（`hasEntityId: true` 的任务，或将实体用作输入/输出的流）
   - 用 `mcp__im_logic__resolve_entity_schema` 取得实体特有的输入/输出类型定义
   - 覆盖模板的 `dataMapMetadata.inputDataDefinition` / `outputDataDefinition`（用于流输入/输出时作为类型定义转用）
   - **不要向 `mcp__im_logic__resolve_entity_schema` 传入猜测的 `entityId`**（避免挂起；务必传入通过 `mcp__im_logic__list_entities` 定位的值）
   - 用户定义任务不需要

3. `mcp__im_logic__list_mapping_functions(query?)` ⇒ `mcp__im_logic__get_mapping_function(name)`  ※ 仅在需要值的转换・加工时
   - 取得映射中使用的函数的名称・参数定义
...
- 因此，调用 `mcp__im_logic__get_task_template` / `mcp__im_logic__get_mapping_function` / `mcp__im_logic__resolve_entity_schema` 时，不要传入猜测的不存在 ID。
- 先用一览类端点（`mcp__im_logic__list_task_types` / `mcp__im_logic__list_mapping_functions`）定位真实存在的 `keyId` / `name`，仅将这些值传入详情取得类端点。
- 对于实体（`hasEntityId: true`）同样，仅向 `mcp__im_logic__resolve_entity_schema` 传入真实存在的 `entityId`。
