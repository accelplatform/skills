...
拥有 Web API Maker 所要求的 `@Path`/HTTP 方法注解的类，在官方文档・javadoc 中被称为「服务类（Service class）」，但**本项目的命名规约（`.github/instructions/java-naming.instructions.md`）为承担该角色的类添加 `Endpoint` 后缀。** 以下模式在注意与该 Web API Maker 术语差异的同时，统一采用 `Xxx` + `Endpoint` / `Xxx` + `EndpointFactory` 的命名。
