...
A JSSP presentation page displayed as a portlet is invoked directly by the portal feature via `b_m_portlet_info.path`, and — unlike a normal screen — does not go through the routing table under `routing-jssp-config/`. Therefore, **do not create** routing configuration or routing authorization via `file-mapping` / `<authz uri="service://...">`. Access control is handled solely by the `im-portal-portlet` / `im-portal-portlet-editmode` authz policies covered in this reference. See `.github/skills/jssp-page-generator/assets/simple-portlet.md` and the "Exception Rules for Screens Not Called Through the Routing Table" section in `.github/instructions/jssp-file-structure.instructions.md` for details.
