...
Those go under `src/main/jssp/src/{feature}/sql/` (see `{{AGENT_RULES}}/jssp-2way-sql.instructions.md`).
...
```jsonc
{
  ...
  "authzPolicies": [
    { "resource": "any-app-content-maintenance", "type": "service", "action": "execute",
      "subject": "S(b_m_role:tenant_manager)", "effect": "PERMIT" },
    { "resource": "any-app-content-maintenance", "type": "service", "action": "execute",
...
Main use case: when you want to **control execution order within the initial setup** between LogicDesigner routing and the authz policies that target it. See [reference/logic-import.md](reference/logic-import.md#injection-order-of-authz-policies-for-routing) for details.
