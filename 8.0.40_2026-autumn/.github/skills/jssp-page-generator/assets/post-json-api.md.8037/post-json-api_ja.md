...
- エラーコード命名は `.github/instructions/jssp-error-handling.instructions.md` に従う
...
`@typedef {Error & {code, httpStatus}} ApiError` をファイル単位で宣言すると `tsc` の `TS2300 Duplicate identifier` を引き起こす。**インライン `@type` 注釈でキャストするのが本テンプレートの標準形**。詳細は `.github/instructions/jssp-error-handling.instructions.md` を参照。
...
- `.github/instructions/jssp-security.instructions.md` - 入力検証の全体方針
