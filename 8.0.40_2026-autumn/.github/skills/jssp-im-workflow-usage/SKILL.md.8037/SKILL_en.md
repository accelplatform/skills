...
This skill generates application/approval screens (`.html` + `.js`) and action processing (`.js`). See the "Convention File List (One-Line Summary + Scope Tag)" in `.github/instructions/README.md` for the full picture. Convention-specific priorities for this skill:
...
> **Note**: IM-Workflow screens are invoked directly by the workflow engine via the `scriptPath` in the XML, so they do not go through the routing table. This means they follow **a separate set of rules from `.github/instructions/jssp-file-structure.instructions.md`'s `view/{view}.js` (snake_case file name unique per screen) convention** — the difference comes from a different invocation source. Follow the rules of this section (the exception is also documented on the `.github/instructions/jssp-file-structure.instructions.md` side as "Exception Rules for Screens Not Called Through the Routing Table").
...
- **Presentation pages (.html) must follow the coding conventions in `.github/instructions/jssp-presentation-page.instructions.md`**. Validation implementation compliance is especially mandatory. After generation, perform a self-check using `reference/screen-generation-checklist.md`.
...
```bash
node .github/skills/jssp-im-workflow-usage/scripts/validate-workflow-code.js src/main/jssp/src/{feature}/
```
...
```bash
node .github/skills/jssp-page-generator/scripts/validate-ddl.js src/main/storage/system/products/import/basic/{feature}/{version}/
```

### Step 3: Manual Check (`post-generation-verification.md`)

Execute all steps in `.github/skills/jssp-page-generator/reference/post-generation-verification.md`.
...
2. For each component, **open `.github/skills/jssp-imds-theme/reference/imds-html-{component}.md` with the Read tool** to re-verify (do not rely on memory)
