...
本スキルは申請画面・承認画面（`.html` + `.js`）およびアクション処理（`.js`）を生成する。全体像は `.github/instructions/README.md` の「規約ファイル一覧（一行要約 + 適用範囲タグ）」を参照。本スキル特有の重要度:
...
| 生成対象 | テンプレート | 配置先 |
|---------|------------|-------|
| 処理対象者プラグイン (.js)（SSJS 実装） | `assets/simple-authority-exec-event-listener.md` | `{機能名}/workflow/plugin/` |
| 処理対象者プラグイン（IM-LogicDesigner 連携） | `assets/logic-flow-authority-plugin.md` | `src/main/storage/public/im_logic/` |
...
- `.github/skills/base-im-workflow-generator/reference/node-types.md` - ノード種別・権限プラグイン一覧（関連スキル）
...
- 「到達処理を作成して」
- 「処理対象者プラグインを実装して」
- 「IM-LogicDesigner で処理対象者を決定したい」
- 「ロジックフローで承認者を動的に決定したい」
- 「案件削除リスナーを追加して」
...
> **注記**: IM-Workflow の画面はワークフローエンジンが XML の `scriptPath` から直接呼び出すため、ルーティングテーブルを介さない。よって `.github/instructions/jssp-file-structure.instructions.md` の `view/{view}.js`（snake_case の画面ごとの固有名）規約とは **呼び出し元が異なる別系統の規約** が適用される。本セクションの規約に従うこと（`.github/instructions/jssp-file-structure.instructions.md` 側にも「ルーティングテーブル経由で呼ばれない画面の例外規約」として明記済み）。
...
- アクション処理・案件処理・分岐条件処理はすべてファンクションコンテナであるため、コーディング規約（`let` の使用、命名規則等）に従うこと。詳細は `.github/skills/jssp-page-generator/reference/` 配下を参照
- テンプレートは必要に応じてカスタマイズ
...
- **プレゼンテーションページ（.html）は `.github/instructions/jssp-presentation-page.instructions.md` のコーディング規約に従うこと**。特にバリデーション実装は規約準拠が必須。生成完了後、`.github/skills/jssp-im-workflow-usage/reference/screen-generation-checklist.md` でセルフチェックを行うこと
...
`.github/skills/jssp-page-generator/reference/post-generation-verification.md` の全ステップを実行する。
...
申請画面・承認画面・詳細画面・確認画面など **`.html` を生成した場合は必ず実行する**。`validate-workflow-code.js` は `imds-selectbox` のような既知の誤クラス名は検出するが、**reference 全体との完全な突き合わせはカバーしない**ため、`.github/skills/jssp-imds-theme/reference/` を引いて目視で再確認する。
...
2. コンポーネントごとに `.github/skills/jssp-imds-theme/reference/imds-html-{component}.md` を **Read ツールで開いて再確認する**（記憶に頼らない）
