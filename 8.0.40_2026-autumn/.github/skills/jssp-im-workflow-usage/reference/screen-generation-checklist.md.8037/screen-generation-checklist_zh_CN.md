...
- [ ] 符合 `.github/instructions/jssp-presentation-page.instructions.md` 规范的验证函数组（`clearValidationError`、`showValidationError`、`createRequest`、`getValidationErrors`、`resetValidationError`、`validateCurrentStep`）按此顺序定义
...
- [ ] **条件必填**（根据特定输入值在必填/可选之间切换）字段的标签不以静态方式附加 `imds-required-label-required`，而是通过以下模式进行动态控制：
  ...
  - id 命名规约的详情请参阅 `.github/instructions/jssp-presentation-page.instructions.md`「id 属性的命名规约」
...
- [ ] 使用 **workflowOpenPage 方式**进行临时保存（`workflowOpenPage('1')`）时，在 `init` 中如 `$imwUserDataId = request['imwUserDataId'] || Identifier.get();` 那样**在应用侧对 userDataId 进行编号**（用户数据 ID 是临时保存信息的键，因此必须在用户内容侧编号。若不编号，新规临时保存时键会为空，重新显示时无法恢复输入内容）
- [ ] 处理模态方式（`showTemporarySave`）时内部会自动编号，因此无需上述显式编号（直接传递 request 的 userDataId）
