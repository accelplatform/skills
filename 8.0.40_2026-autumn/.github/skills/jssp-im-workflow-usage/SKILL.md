...
description: intra-mart IM-Workflow 連携プログラムを新規生成する。アクション処理（申請・承認・否認・差戻し）、案件開始/終了処理、分岐条件処理、申請画面、承認画面の実装パターンを提供する。ワークフロー、申請画面、承認画面、アクション処理、案件処理、分岐条件、稟議、承認フロー、と言及されたときに使用。ワークフロー関連の実装にはこのスキルを使うこと。ジョブスケジューラの定期バッチ処理は jssp-im-job-generator を使うこと。
...
本スキルは申請画面・承認画面（`.html` + `.js`）およびアクション処理（`.js`）を生成する。全体像は `instructions/README.md` の「規約ファイル一覧（一行要約 + 適用範囲タグ）」を参照。本スキル特有の重要度:

| 規約 | 取り扱い |
|------|---------|
| `jssp-presentation-page.instructions.md` | 🟢 **必読** — 申請・承認画面の HTML 構造・バリデーション・id 命名 |
| `jssp-function-container.instructions.md` | 🟢 **必読** — `init()` 構造・workflowOpenPage パラメータ受け取り |
| `jssp-naming.instructions.md` / `jssp-code-style.instructions.md` / `jssp-file-structure.instructions.md` | 🟢 必読 |
| `jssp-error-handling.instructions.md` / `jssp-security.instructions.md` | 🟢 必読 |
| `jssp-2way-sql.instructions.md` | 🔴 **画面側では原則不要**（申請画面 / 承認画面は `workflowOpenPage` で送信するだけで DB 操作なし）。アクション処理側で `db.executeByTemplate` を使う場合のみ参照 |
| `jssp-logging.instructions.md` | 🟡 アクション処理でログ実装時のみ |
...
| 生成対象 | テンプレート | 配置先 |
|---------|------------|-------|
| 申請画面 (.html / .js) | `assets/simple-apply-screen.md` | `{機能名}/workflow/apply/` |
| 承認画面 (.html / .js) | `assets/simple-approve-screen.md` | `{機能名}/workflow/approve/` |
...
**注意:** IM-Workflow の申請・承認画面は、コンテンツ定義で JSSP パスを直接指定するため、ルーティングテーブル（XML）は不要。
...
> **注記**: IM-Workflow の画面はワークフローエンジンが XML の `scriptPath` から直接呼び出すため、ルーティングテーブルを介さない。よって `instructions/jssp-file-structure.instructions.md` の `view/{view}.js`（snake_case の画面ごとの固有名）規約とは **呼び出し元が異なる別系統の規約** が適用される。本セクションの規約に従うこと（`instructions/jssp-file-structure.instructions.md` 側にも「ルーティングテーブル経由で呼ばれない画面の例外規約」として明記済み）。
...
- **プレゼンテーションページ（.html）は `instructions/jssp-presentation-page.instructions.md` のコーディング規約に従うこと**。特にバリデーション実装は規約準拠が必須。生成完了後、`reference/screen-generation-checklist.md` でセルフチェックを行うこと
...
```bash
node .github/skills/jssp-im-workflow-usage/scripts/validate-workflow-code.js src/main/jssp/src/{機能名}/
```
...
#### `<imart type="workflowOpenPage">` 関連の追加チェック
...
| 1 | `tempSave` | `{機能名}/workflow/tempsave/` | 一時保存画面 |
| 2 | `applyTask` | `{機能名}/workflow/apply_task/` | 申請（起票案件）画面 |
