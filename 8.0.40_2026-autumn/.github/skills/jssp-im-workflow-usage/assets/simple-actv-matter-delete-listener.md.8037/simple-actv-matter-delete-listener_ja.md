# ワークフロー 未完了案件削除リスナーテンプレート

## 概要

IM-Workflow の未完了案件削除処理リスナープログラムのテンプレート。
画面を持たず、未完了案件の削除時に自動実行されるバッチ的な処理である。
...
src/main/jssp/src/{機能名}/workflow/
  └── actv_matter_delete_listener.js   # 未完了案件削除リスナー
```
...
## 未完了案件削除リスナー（actv_matter_delete_listener.js）
 ...
 * @description 未完了案件の削除時に実行されるリスナープログラムです。
...
function execute(loginGroupId, localeId, systemMatterId, userDataId) {
    ...
    logger.info('[actvMatterDelete] 未完了案件削除処理開始 systemMatterId={}', systemMatterId);
    try {
        processBusinessLogic(systemMatterId, userDataId);
        logger.info('[actvMatterDelete] 未完了案件削除処理完了 systemMatterId={}', systemMatterId);
    } catch (e) {
        logger.error('[actvMatterDelete] エラーが発生しました。systemMatterId={}, error={}', systemMatterId, e.message);
...
- **未完了案件削除リスナー**: [assets/simple-actv-matter-delete-listener.md](assets/simple-actv-matter-delete-listener.md)
  - 未完了案件の削除時に実行されるリスナー処理
  - 引数は `parameter` オブジェクトではなく個別の値で渡される
  - DB トランザクションは張らないこと

### 生成時の指示例

ユーザが「ワークフローの未完了案件削除リスナーを作成して」と依頼した場合、この assets のコードを参考にして適切にカスタマイズして生成する。
