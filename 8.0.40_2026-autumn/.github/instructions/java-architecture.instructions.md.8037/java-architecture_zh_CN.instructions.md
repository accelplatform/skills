...
5. 违反示例及修正示例请参考 `.github/skills/java-im-architecture/references/anti-patterns.md`
...
贯穿全部分层的实现示例（DDL → Entity → DAO → Repository → Service → UseCase → Endpoint）请参考 `.github/skills/java-im-architecture/references/full-stack-example.md`。

## 命名规约

类名・方法名・变量名的基本规则请参考 `java-naming.instructions.md`。本规约仅补充跨层的类别命名。
...
- 异常转换应将下层异常包装为上层异常，并保留 cause（服务层详情请参考 `java-service-layer.instructions.md`）
- 是否输出日志取决于异常类型。详情请参考 `java-logging.instructions.md` 中的「按异常类型判断日志级别」
...
- 完整的工厂实现模板请参考 `.github/skills/java-im-architecture/SKILL.md`
...
- **必须** 不得将敏感信息（密码、令牌、个人数据）输出到日志（详情请参考 `java-logging.instructions.md`）
- **必须** 在表现层校验并净化所有用户输入
...
类・方法的 JavaDoc 请参考 `java-javadoc.instructions.md`。
...
- `java-naming.instructions.md` - 命名规约
- `java-code-style.instructions.md` - 编码风格
- `java-javadoc.instructions.md` - JavaDoc 规约
- `java-logging.instructions.md` - 日志输出规约
- `java-service-layer.instructions.md` - 服务层实现规约
- `java-entity.instructions.md` - Entity 类规约
- 常见反模式集・贯穿全层的实现示例请参考 `.github/skills/java-im-architecture/SKILL.md`
