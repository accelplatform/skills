...
未指定时会通过 `NameConverter` 从类名自动转换，但本项目要求必须显式指定（参见 `.agents/requirements/java-entity/AGENTS.md`）。
...
根据 `.agents/requirements/java-entity/AGENTS.md`，本项目禁止使用 `GenerationType.APPLICATION` 以外的方式。
