...
- **处理对象者插件（自定义实现）**：职位・组织・角色等标准插件（通过 `node.plugin.suffix` 指定）已支持，但通过 SSJS/Java 完全自定义实现的处理对象者插件的注册途径，很可能与本技能生成的 route/flow 导入 XML 是不同的体系，尚未验证（参见 `.agents/skills/jssp-im-workflow-usage/assets/simple-authority-exec-event-listener.md`）
