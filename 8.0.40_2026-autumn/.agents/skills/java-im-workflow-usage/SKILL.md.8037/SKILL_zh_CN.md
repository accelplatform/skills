...
| 编码规约 | `.agents/requirements/` 下的 jssp 系规约一套 | `.agents/requirements/` 下的 java 系规约一套（`java-naming.md` / `java-code-style.md` / `java-javadoc.md` / `java-logging.md`） |
...
| 规约 | 处理方式 |
|------|----------|
| `.agents/requirements/java-naming/AGENTS.md` | 🟢 **必读** — 包・类・方法命名 |
...
| `.agents/requirements/jssp-error-handling/AGENTS.md` | 🟡 **仅作为思路参考**（错误代码体系的思想。实现时需转换为 Java 的异常机制） |
...
若项目已有既定的 Java 包规约，则遵循该规约。若没有，请仿照 `.agents/requirements/java-naming/AGENTS.md` 的示例（`jp.co.intra_mart.sample.service` 等），以下作为默认值。**默认值终究只是默认值，若用户有明确指定，则以其指示为准。**
...
`{种类}` 目录的对应关系（与 JSSP 版 `.agents/requirements/jssp-file-structure/AGENTS.md` 中 `workflow/` 下的惯例保持一致）：
...
遵循 `.agents/requirements/java-naming/AGENTS.md` 的帕斯卡命名规则。后缀与处理种类对应：
...
- 异常消息请遵循 `.agents/requirements/java-javadoc/AGENTS.md` 的规约，用日语进行说明性描述
...
3. 是否符合 `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
