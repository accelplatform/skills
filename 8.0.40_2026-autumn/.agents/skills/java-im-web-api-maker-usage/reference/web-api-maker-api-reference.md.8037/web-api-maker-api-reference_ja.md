...
ファクトリクラスに付与する3点セット。クラス名は本プロジェクトの命名規則（`.agents/requirements/java-naming/AGENTS.md`）に従い `Xxx` + `EndpointFactory` / `Xxx` + `Endpoint` とする（`@ProvideService` というアノテーション名自体は Web API Maker 側の固定仕様であり変更しない。詳細は `SKILL.md` の「アーキテクチャとクラス命名」参照）。
