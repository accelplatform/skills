...
| 授权 | `<authz>` 的 URI/action 指定（可选） | `<authz>` 中必须指定 `uri/action`（通过授权资源控制）（禁止使用 `welcome-all` 跳过授权）。此外，访问令牌的 **scope** 必须与 `<client-resource>` 的 `<scope id>` 一致（AND 评估） |
...
2. **授权（authz）检查**: 在 `<client-resource>` 的 `<authz uri="service://..." action="execute" />` 中指定（**必填**）。通过授权资源 URI/action 按角色／用户单位控制。**需另行准备授权资源的导入资材**（若不准备则始终返回 403）。禁止使用 `welcome-all` 跳过授权（省略 `<authz>` 或回退）

两者都不满足时，平台在调用应用程序的 `init` 之前会返回 401 / 403。
`<authz>` 的写法请参考 `oauth-resources-config.md` 的「`<authz>` 的写法」。
