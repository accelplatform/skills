...
5. 違反例と修正例は `.claude/skills/java-im-architecture/references/anti-patterns.md` を参照
...
全レイヤーを縦断する実装例（DDL → Entity → DAO → Repository → Service → UseCase → Endpoint）は `.claude/skills/java-im-architecture/references/full-stack-example.md` を参照。
...
- ファクトリ実装の完全なテンプレートは `.claude/skills/java-im-architecture/SKILL.md` を参照
...
- 頻出アンチパターン集・全レイヤー縦断実装例は `.claude/skills/java-im-architecture/SKILL.md` を参照
