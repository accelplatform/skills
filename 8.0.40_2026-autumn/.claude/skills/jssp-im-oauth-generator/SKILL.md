...
- `rules/jssp-function-container.md` - 基本構造
- `rules/jssp-error-handling.md` - エラーレスポンス・HTTP ステータス
- `rules/jssp-logging.md` - ログ出力
- `rules/jssp-security.md` - SQL インジェクション・機密情報の取扱い
- `rules/jssp-2way-sql.md` - DB アクセスがある場合
...
**補完時の規約:**
- DB アクセスを伴う場合は `rules/jssp-2way-sql.md` に従い、`src/main/jssp/src/{機能名}/sql/` 配下に SQL ファイルを外部化し `executeByTemplate` を使う
