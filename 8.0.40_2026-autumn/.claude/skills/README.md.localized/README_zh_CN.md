...
- 路由表：不支持对授权资源的反查指示。请从 `welcome-all` 适当地修改。
- 作业：不支持作业定义导入数据的生成。
- 为检查生成物的正确性，会执行 Node.js 脚本。临时使用 `/tmp`。
- IM-Workflow：主定义的 JSSP-API 不在对象范围。仅支持案件获取/操作系。
- IM-Workflow：作为申请人・处理对象，不支持使用 IM-LogicDesigner 插件。
- IM-Workflow：列表显示模式・流程组・媒体・消息不在对象范围。
...
- IM-LogicDesigner：触发器・逻辑流的预览图像生成不在对象范围。
- IM-BloomMaker / ViewCreator / Accel Studio：资源生成不在对象范围。
