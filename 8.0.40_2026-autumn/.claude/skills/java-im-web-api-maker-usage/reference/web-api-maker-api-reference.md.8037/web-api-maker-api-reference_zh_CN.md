...
附加于工厂类的三件套。类名遵循本项目的命名规则（`.claude/rules/java-naming.md`），采用 `Xxx` + `EndpointFactory` / `Xxx` + `Endpoint`（`@ProvideService` 这一注解名本身是 Web API Maker 一侧的固定规格，不可更改。详情参见 `SKILL.md` 中的「架构与类命名」）。
