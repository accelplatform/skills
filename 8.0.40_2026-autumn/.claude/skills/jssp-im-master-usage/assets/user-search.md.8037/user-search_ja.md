...
IM-共通マスタの `imACMSearch` を利用した、ユーザ検索ダイアログの呼び出し実装例。
...
- `prop` でコールバック関数に渡される取得項目を指定する（`user_cd`, `user_name`）
- `type: 'single'` で単一選択モード、`'multiple'` で複数選択モードになる
- `wnd_close: true` で選択後にダイアログを自動的に閉じる
- `additional_disp`, `additional_user_search_name`, `additional_dept` はユーザ検索固有のオプションで、補足情報・検索名・所属を表示する

### 1.3 コールバック関数

検索ダイアログで選択されたユーザの情報を受け取り、フォームのフィールドに反映する。
...
- `result[i].data` 内のフィールド名は `prop` で指定したキーに対応する
...
**ポイント:**
- `input[type="hidden"]` でユーザコードを保持し、サーバ送信時に使用する
...
- `:userCode:`, `:userName:` はプレースホルダーであり、実装時に一意の ID に置き換えること
...
- 複数選択モードでは、コールバックの `result` を変数に保持し、再検索時に `default_selected` パラメータとして渡すことで、選択済みの項目をダイアログ上に復元すること。実装パターンは以下の通り:
  - 保持変数の宣言: `let selectedUser = [];`
  - コールバック内で保存: `selectedUser = result;`
  - ダイアログ起動時に渡す: `default_selected: selectedUser`
