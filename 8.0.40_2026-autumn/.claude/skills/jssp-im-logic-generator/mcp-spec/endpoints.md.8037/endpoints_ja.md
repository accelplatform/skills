...
ツール名は `mcp__im_logic__<操作名>` 形式（snake_case）。
...
[mcp__im_logic__list_task_types.response.json](schemas/mcp__im_logic__list_task_types.response.json) 参照。
...
- `hasEntityId: true` のタスクは、spec 作成時に `mcp__im_logic__resolve_entity_schema` の追加呼び出しが必要（ユーザ定義タスクでは常に `false`）
...
[mcp__im_logic__get_task_template.response.json](schemas/mcp__im_logic__get_task_template.response.json) 参照。
...
- エンティティ系タスク（`hasEntityId: true`）の場合、`dataMapMetadata.inputDataDefinition` / `outputDataDefinition` はエンティティ未指定時のデフォルト（空 or 汎用スキーマ）を返す。実際のエンティティに基づく型は `mcp__im_logic__resolve_entity_schema` で取得して上書きすること
...
- ユーザ定義タスクの `inputDataDefinition` / `outputDataDefinition` はユーザがテナント側で自由に定義したもので、`mcp__im_logic__resolve_entity_schema` での上書きは不要
...
**`mcp__im_logic__resolve_entity_schema` は実在しない `entityId` を渡すとハングし得るため、まず本エンドポイントで実在する `entityId` を特定してから渡すこと。**
...
[mcp__im_logic__list_entities.response.json](schemas/mcp__im_logic__list_entities.response.json) 参照。
...
- フィールド（カラム）定義は含まない軽量サマリ。フィールド定義は `mcp__im_logic__resolve_entity_schema` で取得する（`mcp__im_logic__list_task_types` ⇒ `mcp__im_logic__get_task_template` と同じ二段構え）
...
[mcp__im_logic__resolve_entity_schema.response.json](schemas/mcp__im_logic__resolve_entity_schema.response.json) 参照。
...
- 返される `inputDataDefinition` / `outputDataDefinition` は `mcp__im_logic__get_task_template` で取得したテンプレートの `dataMapMetadata` 内の同フィールドを上書きして使用する
...
[mcp__im_logic__list_mapping_functions.response.json](schemas/mcp__im_logic__list_mapping_functions.response.json) 参照。
...
- 現状約50件。全件取得でも軽量（数KB）
- `argCount` でおおまかな引数の数がわかる。詳細は `mcp__im_logic__get_mapping_function` で取得
...
[mcp__im_logic__get_mapping_function.response.json](schemas/mcp__im_logic__get_mapping_function.response.json) 参照。
...
2. `mcp__im_logic__list_entities(query?, category?)` ⇒ `mcp__im_logic__resolve_entity_schema(keyId, entityId)`  ※ エンティティを使う場合のみ
   - `mcp__im_logic__list_entities` で実在する `entityId` を特定（`hasEntityId: true` のタスク、またはエンティティを入出力に使うフロー）
   - `mcp__im_logic__resolve_entity_schema` でエンティティ固有の入出力型定義を取得
   - テンプレートの `dataMapMetadata.inputDataDefinition` / `outputDataDefinition` を上書き（フロー入出力に使う場合は型定義として転用）
   - **`mcp__im_logic__resolve_entity_schema` に推測の `entityId` を渡さない**（ハング回避。必ず `mcp__im_logic__list_entities` で特定した値を渡す）
   - ユーザ定義タスクでは不要

3. `mcp__im_logic__list_mapping_functions(query?)` ⇒ `mcp__im_logic__get_mapping_function(name)`  ※ 値の変換・加工が必要な場合のみ
...
- そのため、`mcp__im_logic__get_task_template` / `mcp__im_logic__get_mapping_function` / `mcp__im_logic__resolve_entity_schema` を呼ぶ際は、存在しない ID を推測で渡さないこと。
- まず一覧系（`mcp__im_logic__list_task_types` / `mcp__im_logic__list_mapping_functions`）で実在する `keyId` / `name` を特定し、その値のみを詳細取得系に渡すこと。
- エンティティ（`hasEntityId: true`）についても、`mcp__im_logic__resolve_entity_schema` には実在する `entityId` のみを渡すこと。
