...
> **注記**: IM-Workflow の画面はワークフローエンジンが XML の `scriptPath` から直接呼び出すため、ルーティングテーブルを介さない。よって `{{AGENT_RULES}}/jssp-file-structure.md` の `view/{view}.js`（snake_case の画面ごとの固有名）規約とは **呼び出し元が異なる別系統の規約** が適用される。本セクションの規約に従うこと（`{{AGENT_RULES}}/jssp-file-structure.md` 側にも「ルーティングテーブル経由で呼ばれない画面の例外規約」として明記済み）。
...
- **プレゼンテーションページ（.html）は `{{AGENT_RULES}}/jssp-presentation-page.md` のコーディング規約に従うこと**。特にバリデーション実装は規約準拠が必須。生成完了後、`reference/screen-generation-checklist.md` でセルフチェックを行うこと
