...
- **Processing Target User Plugin (SSJS implementation)**: [assets/simple-authority-exec-event-listener.md](assets/simple-authority-exec-event-listener.md)
...
**Choose SSJS plugin when:**
- Complex logic that is difficult to implement in IM-LogicDesigner (multiple API integrations, dynamic queries, etc.)
- Change management aligned with a release cycle is required

**Choose IM-LogicDesigner integration when:**
- Operations where the flow can be modified and published from the administration screen are preferred
- The logic can be covered by existing IM-LogicDesigner tasks (DB search, branching, etc.)

### Generation Instruction Examples

When a user requests "Create a workflow processing target user plugin", confirm which approach to use before proceeding with implementation.
- SSJS implementation → Use the code in these assets as a reference and generate appropriately customized code.
- IM-LogicDesigner integration → Use the spec.json template in `assets/logic-flow-authority-plugin.md` to generate the flow definition.
