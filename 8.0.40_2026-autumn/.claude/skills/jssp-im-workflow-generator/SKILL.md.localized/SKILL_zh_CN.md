...
按案件数据条件动态增减审批者的多段审批，默认使用 **`matterProperties` + `rules` + `branch_start` 节点（分支路线）**。判断流程、典型示例（采购价格 10 万日元以上时追加部门经理审批）以及与其他实现方式（案件开始处理 / MCP `resolveAuthority`）的取舍，请参见 [reference/dynamic-approval-flow.md](reference/dynamic-approval-flow.md)。
...
- 不符合上述任何情况的指示（例："2026/10/01之后入职的用户"等自定义条件），通过 MCP `resolveAuthority` 解决。详见 [mcp-spec/endpoints.md](mcp-spec/endpoints.md)。
...
2. **MCP `resolveAuthority`**：传入审批者的自然语言描述，从结果中获取 `targetCode`。
...
```jsonc
{
  ...
  "screens": {                                 // 个别画面路径指定（省略时从 screenBasePath 自动生成）
    ...
    "tempSave": null,                          //   临时保存画面（pageType=1, null=默认）
...
| 1 | `tempSave` | 生成 `tempsave/index` | 临时保存画面。可通过 `false` 省略 |
| 2 | `applyTask` | **默认省略**（不输出到 XML） | 申请（起票案件）画面。仅用于月报、期初目标设定等通过作业自动起票的定期申请模式。仅在显式指定路径时输出 |
