...
- **Entity classes follow the conventions in `.claude/rules/java-entity.md`.** Public fields, a no-argument constructor, `GenerationType.APPLICATION`, and the like are rules this project defines in that convention document; this skill fulfills them as part of the im_mirage implementation
...
- **Extending `AbstractDAO` causes the audit fields (`createUserCd`/`createDate`/`recordUserCd`/`recordDate`) to be set automatically on insert/update.** Setting them manually is unnecessary and discouraged (also stated explicitly in `.claude/rules/java-entity.md`)
...
| Convention | Handling |
|------|---------|
| `.claude/rules/java-entity.md` | 🟢 **Required reading** — entity class design conventions (public fields, audit fields, type mapping table) |
| `.claude/rules/java-naming.md` | 🟢 **Required reading** — package, class, method, and variable naming |
...
| The `TenantDatabase`/`SharedDatabase` API and 2WaySQL (`executeByTemplate`) in JSSP (script development model) | `jssp-page-generator` + `.claude/rules/jssp-2way-sql.md` (out of scope for this skill) |
...
| The `TenantDatabase`/`SharedDatabase` API and 2WaySQL in JSSP (script development model) | `jssp-page-generator` + `.claude/rules/jssp-2way-sql.md` |
