...
- I want to check the layer structure, dependencies, naming, exception hierarchy, and factory pattern for Java implementation
  - ⇒ `java-im-architecture`
    ...
    - The essential points of the convention are summarized in `.claude/rules/java-architecture.md` for constant reference; this skill is the detailed version (full code templates)
- I want to know the implementation patterns for the service layer (business logic / transaction control)
  - ⇒ `java-im-service-layer`
    ...
    - The essential points of the convention are summarized in `.claude/rules/java-service-layer.md` for constant reference

> For general Java conventions (naming, coding style, JavaDoc, logging, entities), refer to the relevant file listed in `.claude/rules/README.md`. The two skills above only need to be invoked when the full code templates and anti-pattern catalog that accompany the conventions are required.
