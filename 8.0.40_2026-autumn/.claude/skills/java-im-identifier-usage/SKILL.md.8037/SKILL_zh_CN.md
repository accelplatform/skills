...
`.claude/rules` 下目前不存在规定 `IOException` 包装方针的 Java 专用规约（截至2026年）。使用 `get()` 时的异常处理应遵循 `assets/identifier-basic-usage.md` 中的模式。
...
4. 是否符合 `.claude/rules/java-naming.md` / `java-code-style.md` / `java-javadoc.md`
