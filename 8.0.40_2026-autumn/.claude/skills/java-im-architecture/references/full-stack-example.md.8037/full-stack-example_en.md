...
An example implementing all layers, from DDL to the endpoint, for a single business feature (category management).
Shows the package placement of each class and the dependencies between layers.
...
public class StandardCategoryRepository implements CategoryRepository {
    ...
    // ... remove, findAll omitted
}
```

## 8. Service Interface + Implementation (Domain Layer + Infrastructure Layer)
...
## 10. Endpoint (Presentation Layer)
...
```
[Client] GET /api/example/category/CAT001
      │
      ▼
CategoryEndpoint.get("CAT001")
      │ Passes categoryId to the UseCase
      ▼
GetCategoryUseCase.execute("CAT001")
      │ Calls the Service
      ▼
StandardCategoryService.findById("CAT001")
      │ Calls the Repository (wraps exceptions in a ServiceException)
      ▼
...
CategoryDAO.findByCategoryId("CAT001")
      │ sqlManager.getSingleResult() → executes find-by-id.sql
      ▼
