...
- **処理対象者プラグイン（カスタム実装）**: 役職・組織・ロール等の標準プラグイン（`node.plugin.suffix` で指定するもの）は対応済みだが、SSJS/Javaでフルカスタム実装した処理対象者プラグインの登録経路は、本スキルが生成する route/flow インポート XML とは別体系である可能性が高く未検証（`.claude/skills/jssp-im-workflow-usage/assets/simple-authority-exec-event-listener.md` 参照）
