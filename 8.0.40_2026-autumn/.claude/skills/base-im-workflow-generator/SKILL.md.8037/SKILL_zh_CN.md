...
description: 新建 IM-Workflow 导入用 XML 定义文件。根据提示指示（工作流名称、路线模式、审批节点构成）生成包含 contents/route/flow 段的导入 XML。当提及"生成工作流定义"、"创建 WF 导入 XML"、"工作流定义文件"时使用。工作流程序（动作处理、画面）：JSSP 实现请使用 jssp-im-workflow-usage，Java（JavaEE 开发模型）实现请使用 java-im-workflow-usage。
...
},
{
  ...
  "actionProcessImpl": "java"  // Java 类执行（JavaEE 开发模型）。省略时为 "jssp"（脚本执行）
}
```
...
- 指定 `actionProcessImpl: "java"` 时，`actionProcess` 应指定**实现类的完全限定名（FQCN）**，而不是 JSSP 文件路径（实现请使用 `java-im-workflow-usage` 技能）。`pluginId` 将变为 `{exPointId}.pluginJavaExecutor`（参见 [reference/java-class-registration.md](reference/java-class-registration.md)）
...
```jsonc
{
  ...
  "matterEndProcessImpl": "jssp"  // 指定 "java" 时为 Java 类执行（JavaEE 开发模型）。省略时为 "jssp"
}
```

| 字段 | 必须 | 默认值 | 说明 |
|------|------|--------|------|
| `matterEndProcess` | No | 无 | JSSP 实现：脚本路径（无扩展名） / Java 实现：实现类的完全限定名（FQCN）。省略时不注册插件 |
| `matterEndProcessNoTransaction` | No | `false` | `true` 时使用无事务版扩展点 |
| `matterEndProcessImpl` | No | `"jssp"` | 指定 `"java"` 时为 Java 类执行（`pluginId` 变为 `.pluginJavaExecutor`）。实现请使用 `java-im-workflow-usage` 技能 |

- 有事务：`jp.co.intra_mart.workflow.plugin.event.matter.end.process`
- 无事务：`jp.co.intra_mart.workflow.plugin.event.matter.end_no_transaction.process`

**其他插件相关字段（到达处理・案件开始处理・案件退避处理・案件删除监听器）：**

与 `actionProcess` / `matterEndProcess` 相同的模式，到达处理（节点级 `arriveProcess`）・案件开始处理（`matterStartProcess`）・案件退避处理（`matterArchiveProcess`）・未完成/已完成/历史案件删除监听器（`activeMatterDeleteProcess` 等）也都可以以 JSSP/Java 两种实现方式自动注册。字段一览・spec.json 写法・未支持项目（用户程序方式的分支条件・处理对象者插件的自定义实现）请参阅 [reference/lifecycle-plugin-fields.md](reference/lifecycle-plugin-fields.md)。
...
通过 spec 的 `flowSettings` 对象控制 IM-Workflow 流程定义的功能设置（批量处理・附件・自动处理・自动催促等）。省略时使用默认值。字段一览・XML 标签对应表・需在管理画面手动设置的项目请参阅 [reference/flow-settings.md](reference/flow-settings.md)。

### 3. 执行 build-workflow.js

```bash
node .claude/skills/base-im-workflow-generator/scripts/build-workflow.js \
...
```bash
node .claude/skills/base-im-workflow-generator/scripts/validate-workflow.js \
     <输出的 .xml>
```
...
| 技能 | 用途 |
|------|------|
| **base-im-workflow-generator**（本技能） | 生成 WF 定义 XML（contents/route/flow）。同时支持 JSSP 和 Java 两种执行方式（Java 参见 [reference/java-class-registration.md](reference/java-class-registration.md)） |
| jssp-im-workflow-usage | 生成 WF 集成程序（.js/.html，脚本开发模型） |
| java-im-workflow-usage | 生成 WF 集成程序（.java，JavaEE 开发模型）。画面不在范围内 |
