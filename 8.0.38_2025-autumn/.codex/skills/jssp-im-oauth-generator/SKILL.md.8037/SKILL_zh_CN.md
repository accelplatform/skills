...
- `{{AGENT_ROOT}}/requirements/jssp-function-container.md` - 基本结构
- `{{AGENT_ROOT}}/requirements/jssp-error-handling.md` - 错误响应・HTTP 状态码
- `{{AGENT_ROOT}}/requirements/jssp-logging.md` - 日志输出
- `{{AGENT_ROOT}}/requirements/jssp-security.md` - SQL 注入・机密信息处理
- `{{AGENT_ROOT}}/requirements/jssp-2way-sql.md` - 涉及 DB 访问时
...
**补充时的规约:**
- 涉及 DB 访问时遵循 `{{AGENT_ROOT}}/requirements/jssp-2way-sql.md`，将 SQL 文件外置于 `src/main/jssp/src/{功能名}/sql/` 目录下，使用 `executeByTemplate`
