# Company Search Dialog Implementation Example

An implementation example for calling the company search dialog using `imACMSearch` from IM-Common Master.
...
| Field | (imds-theme) | Company name input field |
        ...
        └── form.imds-form
            └── section
                └── imds-field-container
                    └── imds-field (company name)
                        ├── input[type="hidden"]    ... Company code (hidden field)
                        └── imds-textbox-control    ... Company name (with search icon, readonly)
```
...
**Key points:**
- Specify the company search plugin ID `jp.co.intra_mart.master.app.search.tabs.company.list` in `tabs`
- Use `prop` to specify the fields passed to the callback function (`company_cd`, `department_set_cd`, `department_name`)
...
- User search-specific options (such as `additional_disp`) are not needed for company search

### 1.3 Callback Function

Receive the selected company information from the search dialog and reflect it into the form fields.
...
- The company search `data` includes `company_cd`, `department_set_cd`, `department_cd`, `department_name`, and `delete_flag`
...
<div class="imds-field is-horizontal imds-w-15 sample-company">
  <div class="imds-field-label"><label for=":companyName:">Company Name</label></div>
  <div class="imds-field-control">
    <div class="imds-textbox-control">
      <input type="hidden" id=":companyCode:" value="">
      <input type="text" id=":companyName:" placeholder="Select company name" class="imds-textbox" readonly value="">
...
**Key points:**
- Use `input[type="hidden"]` to hold the company code for server submission
...
- `:companyCode:` and `:companyName:` are placeholders; replace them with unique IDs during implementation
...
- In multi-selection mode, store the callback `result` in a variable and pass it as the `default_selected` parameter when reopening the dialog to restore previously selected items. The implementation pattern is as follows:
  - Declare a holding variable: `let selectedCompany = [];`
  - Save in the callback: `selectedCompany = result;`
  - Pass when launching the dialog: `default_selected: selectedCompany`
