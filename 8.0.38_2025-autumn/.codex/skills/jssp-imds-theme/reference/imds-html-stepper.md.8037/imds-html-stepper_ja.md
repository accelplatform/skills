...
- ステッパーの下にコンテンツ（フォーム等）が続く場合は、ステッパーの親要素に下余白（`imds-pb-4` 等）を設定し、コンテンツとの間に十分な間隔を確保する
- ステッパーの左右には余白を付けない（ステッパーは画面幅いっぱいに表示する）
- ステッパーの直上がヘッダ（`imds-header`）の場合、上余白も付けない
