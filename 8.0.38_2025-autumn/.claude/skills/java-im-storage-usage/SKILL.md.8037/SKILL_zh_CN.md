...
| 规约 | 处理方式 |
|------|---------|
| `.claude/rules/java-naming.md` | 🟢 **必读** — 包名、类名、方法名、变量命名 |
...
- **防范路径穿越攻击。** 不要将用户输入直接用作文件名、路径。对包含 `..` 或 `/`、`\` 的输入进行清洗或拒绝（若项目另有 Java 专用安全规约则参考之；若没有，则将 `.claude/rules/jssp-security.md` 的思路套用到 Java 的异常机制中）
...
- 异常可以直接以 `throws IOException` 向外传播，也可以用业务异常包装。错误消息应按照 `.claude/rules/java-javadoc.md` 的规约用日文进行说明性描述
...
5. 是否符合 `.claude/rules/java-naming.md` / `java-code-style.md` / `java-javadoc.md`
