...
入力フォームを実装する際は、必ず `.claude/skills/jssp-page-generator/assets/simple-form.md` の正規パターンに従う。
...
```bash
node .claude/skills/jssp-imds-theme/scripts/validate-imds.js <対象ファイルまたはディレクトリ>
```
...
| 未定義クラス検出 | `imds-*` プレフィックスを持つが `reference/` ・`.claude/rules/` いずれにも記載が無いクラスを `IMDS-U-001`（warning）として通知。typo / 架空クラス（例: `imds-page-header`）を発見できる |
...
5. `node .claude/skills/jssp-imds-theme/scripts/validate-imds.js <生成ファイル>` を実行し PASS を確認
...
4. `node .claude/skills/jssp-imds-theme/scripts/validate-imds.js <生成ファイル>` を実行し PASS を確認
