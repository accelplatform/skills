...
bash .claude/skills/jssp-page-generator/scripts/check-types.sh src/main/jssp/src/{功能名}/
```
...
仅对 grep 命中的类别，使用 Read 工具打开 `.claude/skills/jssp-page-generator/reference/post-generation-verification.md` 中**对应的章节**（指定偏移量进行部分读取，不读取全文）。按照该章节的说明，与目标文件内容进行核对与修正。
