...
```jsonc
{
  ...
  "captions": [
    { "key": "CAP.Z.IWP.SAMPLE.MY.FEATURE.TITLE", "en": "My Feature", "ja": "マイ機能", "zh_CN": "我的功能" }
  ],
  "messages": [
    { "key": "MSG.E.IWP.SAMPLE.MY.FEATURE.SYSTEM.ERROR", "en": "An unexpected error occurred.", "ja": "予期しないエラーが発生しました。", "zh_CN": "发生了意外错误。" }
  ],
  "logMessages": [
    { "key": "E.IWP.SAMPLE.MY.FEATURE.00001", "en": "An error occurred while displaying the screen.", "ja": "画面表示中にエラーが発生しました。", "zh_CN": "显示画面时发生了错误。" }
  ]
}
```
...
|---------|-----------------|------|---------|
| Caption | `caption_<locale>.properties` | Short display strings such as titles, labels, button names | `CAP.Z.IWP.<product>.<feature>.<caption-name>` |
| Message | `message_<locale>.properties` | Error messages, confirmation messages, success messages, etc. | `MSG.<error-type>.IWP.<product>.<feature>.<message-name>` |
| Log message | `log-message_<locale>.properties` | Messages for Logger output | `<error-type>.IWP.<product>.<feature>.<sequence>` |
