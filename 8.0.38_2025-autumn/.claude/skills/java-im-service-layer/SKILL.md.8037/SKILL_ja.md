...
> 規約の要点（ガードレール）は `.claude/rules/java-service-layer.md` にまとめている。本スキルはそこから参照される、完全なコードテンプレートを提供する。
...
- `.claude/rules/java-service-layer.md` - 本スキルの規約要約（常時参照）
- `.claude/rules/java-architecture.md` - アーキテクチャ原則・レイヤ構造
- `.claude/rules/java-entity.md` - エンティティ・ドメインモデル定義
- `.claude/rules/java-logging.md` - ログ実装ルール
