...
- `.claude/rules/jssp-function-container.md` - 基本结构
- `.claude/rules/jssp-error-handling.md` - 错误响应・HTTP 状态码
- `.claude/rules/jssp-logging.md` - 日志输出
- `.claude/rules/jssp-security.md` - SQL 注入・机密信息处理
- `.claude/rules/jssp-2way-sql.md` - 涉及 DB 访问时
- `.claude/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()`
- `.claude/skills/jssp-page-generator/reference/api-account-context.md` - 获取认证用户
...
**补充时的规约:**
- 涉及 DB 访问时遵循 `.claude/rules/jssp-2way-sql.md`，将 SQL 文件外置于 `src/main/jssp/src/{功能名}/sql/` 目录下，使用 `executeByTemplate`
