...
| **授权方式（`<authz>`）** | (A) `welcome-all`（= 省略 `<authz>`，回退至顶部的 `<authz-default mapper="welcome-all" />`） 或 (B) `uri/action` 二选一。详情请参考 `reference/oauth-resources-config.md`「`<authz>` 的写法与 scope 的关系」。**必须向用户确认**（选择后者会同时需要生成授权资源导入资材） |
...
- `{{AGENT_ROOT}}/rules/jssp-function-container.md` - 基本结构
- `{{AGENT_ROOT}}/rules/jssp-error-handling.md` - 错误响应・HTTP 状态码
- `{{AGENT_ROOT}}/rules/jssp-logging.md` - 日志输出
- `{{AGENT_ROOT}}/rules/jssp-security.md` - SQL 注入・机密信息处理
- `{{AGENT_ROOT}}/rules/jssp-2way-sql.md` - 涉及 DB 访问时
  ...
  "resources": [                                // 公开的 REST-API 的 URL 映射（可多个）
    {
      ...
      "authz": "welcome-all",                   // (A) 指定 "welcome-all" 时不输出 <authz>，
                                                //     回退至顶部的 <authz-default mapper="welcome-all" />
                                                //     （省略时行为相同）
                                                // (B) 指定 { "uri": "service://...", "action": "execute" } 时
...
**补充时的规约:**
- 涉及 DB 访问时遵循 `{{AGENT_ROOT}}/rules/jssp-2way-sql.md`，将 SQL 文件外置于 `src/main/jssp/src/{功能名}/sql/` 目录下，使用 `executeByTemplate`
...
| **授权资源的整合** | `<authz>` 指定 `uri/action` 时，相应的授权资源导入资材（由 `jssp-tenant-setup-generator` 生成）是否准备好。选择 `welcome-all`（= 省略 `<authz>`）则跳过 |
...
- 假定请求示例（`curl` 等）
- 使用既有客户端／新建客户端的哪一种
- 授权方式（`welcome-all` 还是 `uri/action`）。若选择 `uri/action`，**须明示接下来需要用 `jssp-tenant-setup-generator` 生成授权资材**
