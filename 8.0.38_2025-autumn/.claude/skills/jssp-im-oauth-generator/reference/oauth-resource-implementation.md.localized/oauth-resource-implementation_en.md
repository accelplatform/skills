...
- Same when outputting logs (use `MaskUtil` in `{{AGENT_RULES}}/jssp-logging.md`)
...
- `{{AGENT_RULES}}/jssp-error-handling.md` - Error response format / HTTP status
- `{{AGENT_RULES}}/jssp-security.md` - SQL injection / XSS countermeasures (applied similarly in OAuth API)
...
- [ ] When using SQL access, is `DbParameter` used for binding (string concatenation forbidden, see `{{AGENT_RULES}}/jssp-2way-sql.md`)?
