...
The service layer implements business logic and orchestrates operations that span multiple repositories. See `java-architecture.md` for the overall layer structure.
...
See `.claude/skills/java-im-service-layer/references/StandardServiceTemplate.java` for the full template.
...
See `.claude/skills/java-im-service-layer/references/MultiRepositoryServiceTemplate.java` for an example that handles multiple repositories in a single transaction.
...
- Exception messages must be in Japanese and include variable values needed for troubleshooting. See `java-logging.md` for whether/at what level to log
...
- `java-architecture.md` - Architecture principles and layer structure
- `java-entity.md` - Entity class conventions
- `java-logging.md` - Logging conventions
- See `.claude/skills/java-im-service-layer/SKILL.md` for the full service implementation templates
