...
服务层实现业务逻辑，并编排跨越多个仓储的操作。整体分层结构请参考 `java-architecture.md`。
...
完整模板请参考 `.claude/skills/java-im-service-layer/references/StandardServiceTemplate.java`。
...
在单个事务中操作多个仓储的示例请参考 `.claude/skills/java-im-service-layer/references/MultiRepositoryServiceTemplate.java`。
...
- 异常消息应使用日语，并包含排查问题所需的变量值。是否输出日志及级别请参考 `java-logging.md`
...
- `java-architecture.md` - 架构原则・分层结构
- `java-entity.md` - Entity 类规约
- `java-logging.md` - 日志输出规约
- 完整的服务实现模板请参考 `.claude/skills/java-im-service-layer/SKILL.md`
