...
- **Entity classes follow the conventions in `.github/instructions/java-entity.instructions.md`.** Public fields, a no-argument constructor, `GenerationType.APPLICATION`, and the like are rules this project defines in that convention document; this skill fulfills them as part of the im_mirage implementation
...
- **Extending `AbstractDAO` causes the audit fields (`createUserCd`/`createDate`/`recordUserCd`/`recordDate`) to be set automatically on insert/update.** Setting them manually is unnecessary and discouraged (also stated explicitly in `.github/instructions/java-entity.instructions.md`)
...
| Convention | Handling |
|------|---------|
| `.github/instructions/java-entity.instructions.md` | 🟢 **Required reading** — entity class design conventions (public fields, audit fields, type mapping table) |
...
| The `TenantDatabase`/`SharedDatabase` API and 2WaySQL (`executeByTemplate`) in JSSP (script development model) | `jssp-page-generator` + `.github/instructions/jssp-2way-sql.instructions.md` (out of scope for this skill) |
...
2. Design and implement the entity class following `.github/instructions/java-entity.instructions.md` (`@Table`/`@Column`/`@PrimaryKey`, the 4 audit fields)
...
1. Whether the entity class complies with `.github/instructions/java-entity.instructions.md` (public fields, no-argument constructor, `GenerationType.APPLICATION`, the 4 audit fields)
...
14. Whether it complies with `.github/instructions/java-naming.instructions.md` / `java-code-style.md` / `java-javadoc.md`
...
| The `TenantDatabase`/`SharedDatabase` API and 2WaySQL in JSSP (script development model) | `jssp-page-generator` + `.github/instructions/jssp-2way-sql.instructions.md` |
