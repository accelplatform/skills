...
`build-setup-import.js` 会根据 `spec.json` 中的 `portletImport.portlets`，向这三张表生成**实际可运行的 INSERT 语句**（并非像 `equip-ddl.sql` 那样仅有注释的占位符），输出到 `<key>_sample-dml.sql`。此外，还会根据 `editable`（详见后文）的值，自动生成**授予租户管理者查看・编辑权限的授权策略**（`<key>-authz-policy.xml`）。

## 不涉及的范围：门户配置与显示设置

以下两张表**不涉及**。它们是关于"将 Portlet 放置在哪个门户的第几列第几行、以何种显示（窗口状态）呈现"的**门户运维侧设置**，与 Portlet 本身的定义性质不同，因此不包含在租户环境设置统一投放的 DML 中。

- `b_m_portlet_layout`（在门户上的配置位置）
- `b_m_portlet_display_set`（窗口显示设置）

如需在门户上进行配置，请在租户环境设置完成后，从门户管理画面手动进行。

## Portlet 不需要路由配置・路由授权

作为 Portlet 显示的 JSSP 展示页面，由门户功能通过 `b_m_portlet_info.path` 直接调用，与普通画面不同，不经过 `routing-jssp-config/` 下的路由表。因此**不得创建**通过 `file-mapping` / `<authz uri="service://...">` 进行的路由配置・路由授权。访问控制仅通过本 reference 涉及的 `im-portal-portlet` / `im-portal-portlet-editmode` 授权策略来实现。详情请参阅 `.github/skills/jssp-page-generator/assets/simple-portlet.md` 以及 `.github/instructions/jssp-file-structure.instructions.md` 中的"不经过路由表调用的画面的例外规约"章节。
  ...
  "portlets": [
    {
      ...
      "path": "portlet_sample/view/index",         // 要显示的展示页面（相对于 src/main/jssp/src/ 的路径，不含扩展名）
      "pageParam": "test=1",                       // 可选。传递给 Portlet 的固定参数（page_param）。不需要时可省略
      "portletModeCd": "portlet_sample_mode",       // 可选。省略时自动生成 "<portletCd>_mode"
      "editable": false,                            // 可选。false（默认）：仅查看 / true：可查看・可编辑（详见后文"查看・编辑权限的切换"）
      "titles": {
...
| 字段 | 必须 | 说明 |
|------|------|------|
| `portletCd` | YES | Portlet 的唯一 CD。指定在租户内不重复的英数字・下划线 ID。**intra-mart 标准的 `b_m_portlet_info.portlet_cd` 为 VARCHAR(20)，因此必须控制在 20 个字符以内**（超过时会在租户环境设置导入时因 `值は型character varying(20)としては長すぎます` 而失败；`build-setup-import.js` 会在超过时于构建阶段报错） |
| `path` | YES | 作为 Portlet 显示的 JSSP 展示页面的路径（与路由配置的 `page` 属性格式相同） |
| `pageParam` | NO | 传递给 Portlet `init(request)` 的固定查询参数字符串。省略时为空字符串 |
...
| `editable` | NO | `false`（默认）：仅查看（除创建者外不可编辑） / `true`：可查看且可编辑。详见"查看・编辑权限的切换" |
| `titles.name` | YES（3 种语言） | Portlet 的显示名称（页头显示的标题） |
| `titles.application` | YES（3 种语言） | Portlet 的分类名称（添加 Portlet 对话框中的分类名） |
...
| `rec_user_cd` | `system` | |
| `rec_date` | 构建执行时刻 | `build-setup-import.js` 执行时的日期时间。**`rec_date` 并非 timestamp 类型，而是 varchar 类型，采用固定的 `yyyy/MM/dd\|HH:mm:ss` 格式**（并非标准 SQL 的日期时间字面量，请注意） |
...
`build-setup-import.js` 将此计算实现为 `computePortletViewHash(portletCd)` / `computePortletEditHash(portletCd)`，并根据 `editable` 的值，将其作为默认策略（对租户管理者授予 `PERMIT`）自动输出到 `<key>-authz-policy.xml` 中（与 `spec.menuGroups` 的默认策略自动授予采用相同机制）。若需要向租户管理者以外的角色・用户授予权限，请在 `spec.authzPolicies` 中显式指定 `type: "im-portal-portlet"` 或 `type: "im-portal-portlet-editmode"`，并将 `resource` 设为上述哈希值（哈希值可通过例如 `node -e "console.log(require('crypto').createHash('sha256').update('im-portal-portlet://' + '<portletCd>').digest('hex'))"` 计算得出）。

### 授权资源（`<key>-authz-resource.xml`）的自动生成

由于 `authz-policy` 不带 `id`，而是直接将上述哈希值写入 `resource` 属性，因此**若不做任何处理，该 Portlet 不会显示在管理画面的授权资源树中**（策略会处于没有对应资源的悬空状态）。为此，`build-setup-import.js` 也会根据 `portletImport.portlets` 自动生成对应的 `authz-resource` 条目，输出到 `<key>-authz-resource.xml`（以及各语言文件）中。
...
- `<parent-group>` 指定 intra-mart 标准内置组 `im-portal-portlet` / `im-portal-portlet-editmode`（无需在 `authzResourceGroups` 中预先定义，与 `http-services` 一样，视为默认已存在）
- 显示名称直接沿用 `titles.application` 的值；编辑模式一侧会附加各语言对应的后缀（`ja`：`（編集モード）` / `en`：` (Edit Mode)` / `zh_CN`：`（编辑模式）`）
- 当 `editable !== true` 时，仅生成查看权限对应的资源；当 `editable === true` 时，还会额外生成编辑权限对应的资源
- 即使 `spec.authzResources` 为空，只要存在 `portletImport.portlets`，也会生成 `<key>-authz-resource.xml`（及各语言文件），并自动作为 `<authz-resource-file>` 纳入 `import-<artifactId>-config-1.xml` 的 `<tenant-master>` 中

另外，[reference/authz-resource.md](authz-resource.md) 中"本项目仅使用 `service://`"的限制，针对的是 `spec.authzResources`（用户手写的授权资源），而本 `portletImport` 自动生成的资源（`im-portal-portlet(-editmode)://` 方案）属于该限制范围之外的特例。

## 与输出文件的整合

`portletImport` 与既有的 `database`（专有表的 DDL/DML）章节输出到同一个 `<key>_sample-dml.sql` 中。即使没有 `spec.database`，仅凭 `portletImport` 也会生成 DML 文件，并自动纳入 `import-<artifactId>-config-1.xml` 的 `<database><insert-file>` 中。DDL（`<key>-ddl_*.sql`）是专有表专用的机制，仅有 `portletImport` 时不会生成。
