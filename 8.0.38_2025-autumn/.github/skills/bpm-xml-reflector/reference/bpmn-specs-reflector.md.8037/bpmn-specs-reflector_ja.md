...
**本ステップは必ず `.github/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js` の `reflector.reflect()`（内部で `reflectProcessIdReplacements()` を呼ぶ）経由で実行すること。
...
反映ロジックは `.github/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js` に実装されている。以下はその概要と呼び出し方である。
