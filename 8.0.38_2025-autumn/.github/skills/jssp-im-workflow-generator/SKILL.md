...
本スキルは **XML 定義ファイル**（`.xml`）のみを生成し、`.js` / `.html` の実装は行わない（実装は `jssp-im-workflow-usage` の責務）。そのため参照すべき規約は最小限。全体像は `instructions/README.md` 参照。
...
| **`undefined` / `null` / 未記載** | デフォルト挙動（上記対応表参照） |
...
`screens` 自体を省略すれば、上記対応表のデフォルトが適用される。サンプル: [examples/straight.spec.json](examples/straight.spec.json)。
