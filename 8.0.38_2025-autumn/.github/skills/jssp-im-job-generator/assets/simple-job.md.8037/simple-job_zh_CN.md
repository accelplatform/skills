...
作业参数通过函数开头的 `@parameter` 注解声明，并通过 `Contexts.getJobSchedulerContext().getParameter()` 按键获取。`execute()` 不接受参数。
...
> ⚠️ 像 `function execute(params)` 那样通过参数接收并调用 `JSON.parse()` 的方式是错误的。请务必通过 `getParameter()` 获取。
