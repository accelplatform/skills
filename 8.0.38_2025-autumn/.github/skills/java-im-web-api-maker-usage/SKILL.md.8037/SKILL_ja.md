...
Web API Maker の公式ドキュメント・javadoc は `@Path`/HTTP メソッドアノテーションを持つクラスを慣習的に「サービスクラス」と呼ぶが、これは Web API Maker 自体の用語である。**本プロジェクトの命名規則（`.github/instructions/java-naming.instructions.md` の「REST API エンドポイント → `Endpoint` サフィックス」）に従い、この役割の Java クラスには `Endpoint` サフィックスを付与する**（`OrderService` ではなく `OrderEndpoint`）。
...
2. パッケージ・ファクトリクラス名・Endpointクラス名を設計（`.github/instructions/java-naming.instructions.md` に準拠。Endpointクラスは `Xxx` + `Endpoint`、ファクトリクラスは `Xxx` + `EndpointFactory`）
...
11. `.github/instructions/java-naming.instructions.md` / `java-code-style.md` / `java-javadoc.md` に準拠しているか
