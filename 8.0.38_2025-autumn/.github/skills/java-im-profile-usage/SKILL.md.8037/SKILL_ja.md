...
`.github/instructions` 配下には例外処理を定めた Java 向け専用規約は存在しない（2026年時点）。`UserProfileImageManager` の例外（`BizApiException`、チェック例外）は、業務例外へのラップ方針を `assets/profile-basic-usage.md` のパターンに従う。
