...
- 日志输出时同样（活用 `{{AGENT_RULES}}/jssp-logging.instructions.md` 的 `MaskUtil`）
...
- `{{AGENT_RULES}}/jssp-error-handling.instructions.md` - 错误响应格式・HTTP 状态
- `{{AGENT_RULES}}/jssp-security.instructions.md` - SQL 注入・XSS 对策（OAuth API 中同样适用）
...
- [ ] 涉及 SQL 访问时是否用 `DbParameter` 绑定（禁止字符串拼接，参考 `{{AGENT_RULES}}/jssp-2way-sql.instructions.md`）？
