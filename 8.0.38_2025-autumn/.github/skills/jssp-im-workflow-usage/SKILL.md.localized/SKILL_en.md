...
description: Generates new IM-Workflow integration programs. Provides implementation patterns for action processing (apply/approve/deny/return), case start/end processing, branching condition processing, application screens, and approval screens. Use when mentioning workflow, application screen, approval screen, action processing, case processing, branching conditions, approval flow. For job scheduler batch processing, use jssp-im-job-generator.
...
| Convention | Handling |
|------------|----------|
| `jssp-presentation-page.instructions.md` | 🟢 **Required** — HTML structure, validation, id naming for the application/approval screens |
| `jssp-function-container.instructions.md` | 🟢 **Required** — `init()` structure, receiving workflowOpenPage parameters |
| `jssp-naming.instructions.md` / `jssp-code-style.instructions.md` / `jssp-file-structure.instructions.md` | 🟢 Required |
| `jssp-error-handling.instructions.md` / `jssp-security.instructions.md` | 🟢 Required |
| `jssp-2way-sql.instructions.md` | 🔴 **Not needed on the screen side** (the application / approval screens only submit via `workflowOpenPage` and have no DB access). Reference only when the action processing side uses `db.executeByTemplate` |
| `jssp-logging.instructions.md` | 🟡 Only when implementing logging in action processing |
...
|------------------|----------|-----------|
| Application screen (.html / .js) | `assets/simple-apply-screen.md` | `{feature}/workflow/apply/` |
| Approval screen (.html / .js) | `assets/simple-approve-screen.md` | `{feature}/workflow/approve/` |
...
**Note:** IM-Workflow application/approval screens specify the JSSP path directly in the content definition, so a routing table (XML) is not needed.
...
> **Note**: IM-Workflow screens are invoked directly by the workflow engine via the `scriptPath` in the XML, so they do not go through the routing table. This means they follow **a separate set of rules from `{{AGENT_RULES}}/jssp-file-structure.instructions.md`'s `view/{view}.js` (snake_case file name unique per screen) convention** — the difference comes from a different invocation source. Follow the rules of this section (the exception is also documented on the `{{AGENT_RULES}}/jssp-file-structure.instructions.md` side as "Exception Rules for Screens Not Called Through the Routing Table").
...
- **Presentation pages (.html) must follow the coding conventions in `{{AGENT_RULES}}/jssp-presentation-page.instructions.md`**. Validation implementation compliance is especially mandatory. After generation, perform a self-check using `reference/screen-generation-checklist.md`.
...
#### Additional Checks Related to `<imart type="workflowOpenPage">`
...
| 1 | `tempSave` | `{feature}/workflow/tempsave/` | Temporary save screen |
| 2 | `applyTask` | `{feature}/workflow/apply_task/` | Application (issued case) screen |
