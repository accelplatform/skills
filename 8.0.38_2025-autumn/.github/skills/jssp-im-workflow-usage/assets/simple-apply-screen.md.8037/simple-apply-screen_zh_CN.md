# 工作流 申请画面模板（workflowOpenPage 方式 / 旧型）
...
> **与模态方式（新型）的取舍**：使用 `imWorkflow.modal.showApply()` / `showTemporarySave()` 的新型请参考 `modal-apply-screen.md`。新规开发推荐使用新型。本模板用于需要 workflowOpenPage 方式（旧型）申请画面的场合。
...
function init(request) {
  ...
  // 作为临时保存键的用户数据ID。新规（request 中没有）时由应用侧采番。
  // IM-Workflow 规格：用户数据ID 是临时保存信息的键，因此必须在用户内容侧采番。
  // （workflowOpenPage 方式没有处理模态方式 showTemporarySave 那样的自动采番）
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      function buildMatterName() {
        return '零件发注申请_' + document.getElementById(':partCode:').value;
      }

      // 将业务数据复制到提交用 hidden 字段
      // 旧方式（workflowOpenPage）中，显示用 input 不附 name，
      // 在提交前将值复制到带 name 的 hidden 字段（hidden 的值作为动作处理的 userParam 传递）。
      // ※ 若直接在显示用 input 上附 name，当与 hidden 或 select/radio 的 name 重复时，
      //   userParam 会变成数组（NativeArray）并在动作处理中报错，因此务必经由 hidden。
...
<imart type="workflowOpenPage"
  ...
  <!-- 业务数据提交用 hidden 字段。显示用 input 不附 name，
       在提交前通过 setUserDataToHidden() 复制值（带 name 的字段作为 userParam 传递）。 -->
...
- **使用临时保存（`workflowOpenPage('1')`）时，请在应用侧采番 userDataId**。
  - 根据 IM-Workflow 规格，用户数据ID 是临时保存信息的键，**必须在用户内容侧采番**（[临时保存的规格](https://document.intra-mart.jp/library/iap/public/im_workflow/im_workflow_specification/texts/detail_guide/projects/temporary_save/index.html)）。
  - 处理模态方式（`showTemporarySave`）在内部自动采番，但 **workflowOpenPage 方式没有自动采番**，因此在 `init` 中如 `request['imwUserDataId'] || Identifier.get()` 这样采番。
  - 若不采番，新规临时保存时键保持为空，即使在申请画面重新显示也无法恢复输入内容。
...
- **验证的实现请遵守 `.github/instructions/jssp-presentation-page.instructions.md` 的规范**（以下为主要规则）：
