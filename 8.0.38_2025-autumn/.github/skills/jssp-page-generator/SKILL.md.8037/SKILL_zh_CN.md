...
- **对于 Portlet 画面（`assets/simple-portlet.md`），不生成路由配置。** Portlet 由门户功能（`b_m_portlet_info.path`）直接调用，不经过路由表（详见 `.github/instructions/jssp-file-structure.instructions.md` 中的"不经过路由表调用的画面的例外规约"章节）
