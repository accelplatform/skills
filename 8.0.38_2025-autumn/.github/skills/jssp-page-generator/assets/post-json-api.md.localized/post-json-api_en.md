...
- Follow `{{AGENT_RULES}}/jssp-error-handling.instructions.md` for error code naming.
...
In production environments apply proper authorization settings instead of `welcome-all`.
...
Declaring `@typedef {Error & {code, httpStatus}} ApiError` per file triggers `tsc` `TS2300 Duplicate identifier`. **Casting with an inline `@type` annotation is the canonical form of this template.** See `{{AGENT_RULES}}/jssp-error-handling.instructions.md` for details.
...
- `{{AGENT_RULES}}/jssp-error-handling.instructions.md` - Error code naming and API error response format
- `{{AGENT_RULES}}/jssp-security.instructions.md` - Overall input validation policy
