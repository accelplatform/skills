/**
 * SystemStorage アクセスクラス。
 *
 * storage/system ディレクトリ内のファイル・フォルダにアクセスします。
 *
 * @since 8.0.37 (2025 Spring)
 * @see https://api.intra-mart.jp/iap/apilist-ssjs/doc/platform/SystemStorage/index.html
 */
declare class SystemStorage {
  /**
   * 指定したパスを示す、SystemStorage クラスのインスタンスを作成します。
   *
   * @param path システムストレージのパス
   */
  constructor(path: string);

  /**
   * 指定した親・子のパスを示す、SystemStorage クラスのインスタンスを作成します。
   *
  ...
  /**
   * 指定した Storage と同じパスを示す、SystemStorage クラスのインスタンスを作成します。
   *
   * @param storage システムストレージ
   */
  ...
  /**
   * ディレクトリ内のディレクトリを示す SystemStorage コレクションを取得します。
   *
   * @param recursive true の場合、再帰的に探索します
   * @return SystemStorage の配列
   */
  ...
  /**
   * ディレクトリ内のファイルを示す SystemStorage コレクションを取得します。
   *
   * @param recursive true の場合、再帰的に探索します
   * @return SystemStorage の配列
   */
  ...
  /**
   * 親の SystemStorage を返します。
   *
   * @return 親ディレクトリの SystemStorage
   */
  ...
  /**
   * 対象の SystemStorage からの相対パスを返します。
   *
   * @param target 基準となる SystemStorage
   * @return 相対パス文字列
   */
  getRelativePath(target: SystemStorage): string;

  /**
   * ルートの SystemStorage を返します。
   *
   * @return ルートディレクトリの SystemStorage
   */
  ...
  /**
   * ディレクトリ内のファイルおよびディレクトリを示す SystemStorage コレクションを取得します。
   *
   * @param filterOrRecursive Boolean の場合は再帰探索フラグ、Function の場合はフィルタ関数
   * @return SystemStorage の配列
   */
  listStorages(filterOrRecursive?: boolean | ((path: string) => boolean)): SystemStorage[];

  /**
   * ストレージ内容を取得します。
   *
   * @deprecated openAsBinary() を使用してください
   * @return ファイル内容の文字列
   */
  load(): string;
  ...
  /**
   * 相対パスを解決した SystemStorage を返します。
   *
   * @param other 相対パス
   * @return 解決された SystemStorage
   */
