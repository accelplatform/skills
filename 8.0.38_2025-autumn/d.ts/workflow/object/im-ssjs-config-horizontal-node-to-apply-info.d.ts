/**
 * 設定対象横配置ノード情報（申請用）オブジェクト。
 *
 * 申請処理時に設定可能な横配置ノードが存在する場合に、その横配置ノードの情報を保持します。
 *
 * 値の取得時にのみ使用します（新規作成・更新・削除には使用しません）。
 *
 * @since 8.0.37 (2025 Spring)
 * @see https://api.intra-mart.jp/iap/apilist-ssjs/doc/im_workflow/ConfigHorizontalNodeToApplyInfo/index.html
 */
interface ConfigHorizontalNodeToApplyInfo extends ConfigNodeInfo {
  // --- NULLABLE プロパティ ---
