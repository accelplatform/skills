 ...
 * 未完了案件情報（参照用）オブジェクト。
 *
 * 参照一覧用の未完了案件情報を取得する処理で、処理結果である案件情報を保存するためのオブジェクトです。
 * 参照者情報は「imw_t_actv_matter_handle_user」から、
 * 案件情報は「imw_t_actv_matter」「imw_t_actv_matter_locale」から取得します。
 *
 * 値の取得時にのみ使用します（新規作成・更新・削除には使用しません）。
 *
 * @since 8.0.37 (2025 Spring)
 * @see https://api.intra-mart.jp/iap/apilist-ssjs/doc/im_workflow/ActvMatterRefInfo/index.html
 */
interface ActvMatterRefInfo {
  // --- NOT NULL プロパティ ---
