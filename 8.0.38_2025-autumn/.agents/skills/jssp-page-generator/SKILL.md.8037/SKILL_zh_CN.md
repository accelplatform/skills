...
- [ ] [jssp-2way-sql](../../../requirements/jssp-2way-sql/AGENTS.md) 已参考并理解内容
...
本技能同时生成 `.js`（函数容器）+ `.html`（展示页面），因此涉及的规约较多。全局视图请参阅 `.agents/requirements/README.md` 中的「规约文件一览（一行摘要 + 适用范围标签）」。本技能特有的重要度：
...
- **画面是否作为门户页面的 Portlet（部件），可能在同一门户页面上多重配置** → 若是，使用 `assets/simple-portlet.md`（其实现方针与普通画面不同：不含页头、页脚，`$data` 通过 IIFE 进行作用域隔离等）
...
| Portlet 画面（门户组件，可能多重配置） | `assets/simple-portlet.md` |
...
| 组件 | 参考文件 |
|------|---------|
| 文本框 | `.agents/skills/jssp-imds-theme/reference/imds-html-textbox.md` |
| 文本域 | `.agents/skills/jssp-imds-theme/reference/imds-html-textarea.md` |
| 下拉选择 | `.agents/skills/jssp-imds-theme/reference/imds-html-select.md` |
| 复选框 | `.agents/skills/jssp-imds-theme/reference/imds-html-checkbox.md` |
| 单选按钮 | `.agents/skills/jssp-imds-theme/reference/imds-html-radio.md` |
| 按钮 | `.agents/skills/jssp-imds-theme/reference/imds-html-button.md` |
| 表格 | `.agents/skills/jssp-imds-theme/reference/imds-html-table.md` |
| 对话框 | `.agents/skills/jssp-imds-theme/reference/imds-html-dialog.md` |
| 对话框 + 表单（新建、编辑等） | `.agents/skills/jssp-imds-theme/reference/imds-html-dialog-form.md` |
| 分页 | `.agents/skills/jssp-imds-theme/reference/imds-html-pagination.md` |
| 字段（带标签） | `.agents/skills/jssp-imds-theme/reference/imds-html-field.md` |
| 字段组 | `.agents/skills/jssp-imds-theme/reference/imds-html-field-group.md` |
| 标签页 | `.agents/skills/jssp-imds-theme/reference/imds-html-tabs.md` |
| 折叠面板 | `.agents/skills/jssp-imds-theme/reference/imds-html-accordion.md` |
| 日历输入 | `.agents/skills/jssp-imds-theme/reference/imui-html-calendar.md` |
| 横幅消息 | `.agents/skills/jssp-imds-theme/reference/imds-html-banner-message.md` |
| 内联消息 | `.agents/skills/jssp-imds-theme/reference/imds-html-inline-message.md` |

其他组件存放于 `.agents/skills/jssp-imds-theme/reference/` 目录下。

---

### 步骤 4：生成功能容器和路由

参考 `.agents/requirements/jssp-function-container/AGENTS.md` 和 `.agents/requirements/jssp-presentation-page/AGENTS.md` 生成代码。

- 在 `src/main/jssp/src/{功能名}/` 下生成功能容器（.js）
- **对于 Portlet 画面（`assets/simple-portlet.md`），不生成路由配置。** Portlet 由门户功能（`b_m_portlet_info.path`）直接调用，不经过路由表（详见 `.agents/requirements/jssp-file-structure/AGENTS.md` 中的"不经过路由表调用的画面的例外规约"章节）
- 除此之外的画面，根据需要生成路由配置（.xml）
  - 每个 `file-mapping` **必须明示** `<authz uri="service://{功能名}/{处理}" action="execute" />`。**原则上禁止**通过 `welcome-all` / `<authz-default>` 跳过授权
  - 此处引用的授权资源（`service://...`）**必须通过 `jssp-tenant-setup-generator` 定义**（policy / resource / resource-group / subject-group）。若未定义即部署，对目标 URL 的访问将始终被 **403** 拒绝
...
- **省略资源中包含的 JSDoc 注释（`/** ... */`）或分节注释（`// ===...===`）**。即使看似冗余，规约（`.agents/requirements/jssp-function-container/AGENTS.md`）也将其视为必须项。保留它们可以使各函数的意图在代码中得到明确表达，便于后续审查和修改。资源中的注释原则上应照抄；若需修改，仅可将内容改写为对应功能的描述，不得删除。
