...
This skill generates **only XML definition files** (`.xml`); it does not implement `.js` / `.html` (that is the responsibility of `jssp-im-workflow-usage`). Therefore the set of conventions to reference is minimal. See `.agents/requirements/README.md` for the full picture.
...
```bash
node .agents/skills/jssp-im-workflow-generator/scripts/build-workflow.js \
...
```bash
node .agents/skills/jssp-im-workflow-generator/scripts/validate-workflow.js \
     <output .xml>
```
