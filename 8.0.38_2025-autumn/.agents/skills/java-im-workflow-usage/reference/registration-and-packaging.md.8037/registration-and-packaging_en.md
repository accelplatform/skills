...
The "Package Structure" section of `.agents/requirements/java-naming/AGENTS.md` describes a division by layer (`entity` / `service` / `repository`, etc.), but for workflow integration programs, in line with the JSSP version's convention of separating directories by feature (`{feature-name}/workflow/...`), this skill adopts a structure where **a processing-type directory is placed under a feature-specific sub-package** (see the "Placement Convention" section of `SKILL.md`). If the project has an existing package structure convention, prioritize that instead.
