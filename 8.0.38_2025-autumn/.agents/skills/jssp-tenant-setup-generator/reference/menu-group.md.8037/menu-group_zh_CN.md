...
> **租户管理员（`tenant_manager`）由 `build-setup-import.js` 自动授予所有菜单组**，因此无需在 `authzPolicies` 中列出。如下例所示，仅记述**其他目标角色／用户**（设计书或提示指示的对象）。详见 [authz-policy.md](authz-policy.md) 的「默认策略」。
