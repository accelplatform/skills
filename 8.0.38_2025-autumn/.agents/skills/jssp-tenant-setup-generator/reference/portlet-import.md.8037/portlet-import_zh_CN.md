...
作为 Portlet 显示的 JSSP 展示页面，由门户功能通过 `b_m_portlet_info.path` 直接调用，与普通画面不同，不经过 `routing-jssp-config/` 下的路由表。因此**不得创建**通过 `file-mapping` / `<authz uri="service://...">` 进行的路由配置・路由授权。访问控制仅通过本 reference 涉及的 `im-portal-portlet` / `im-portal-portlet-editmode` 授权策略来实现。详情请参阅 `.agents/skills/jssp-page-generator/assets/simple-portlet.md` 以及 `.agents/requirements/jssp-file-structure/AGENTS.md` 中的"不经过路由表调用的画面的例外规约"章节。
