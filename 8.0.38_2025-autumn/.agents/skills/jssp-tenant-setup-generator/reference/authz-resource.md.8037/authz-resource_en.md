...
`spec.authzResources` (the hand-written authorization resources described in this section) uses only `service://`.
...
### Exception: authz resources auto-generated by `portletImport`

When `spec.portletImport.portlets` is specified, an authorization resource using the `im-portal-portlet://<portletCd>` / `im-portal-portlet-editmode://<portletCd>` scheme is auto-generated into `<key>-authz-resource.xml` (no need to hand-write it into `spec.authzResources`). This is a special-case exception to the `service://`-only rule for `spec.authzResources` — a mechanism to make the hash-value-in-`resource`-attribute style used by `authz-policy` (which has no `id`) visible in the admin screen. See [portlet-import.md](portlet-import.md#auto-generation-of-the-authz-resource-key-authz-resourcexml) for details.
