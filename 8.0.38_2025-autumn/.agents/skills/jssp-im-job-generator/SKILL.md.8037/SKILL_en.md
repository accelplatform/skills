...
This skill generates batch processing (`.js` only; no screen). See `.agents/requirements/README.md` for the full picture.
...
```bash
node .agents/skills/jssp-page-generator/scripts/validate-jssp-code.js src/main/jssp/src/{feature-name}/
```

### Step 2: Manual Check

Execute steps 1–3 in `.agents/skills/jssp-page-generator/reference/post-generation-verification.md` (step 4 screen verification is not needed for jobs).
