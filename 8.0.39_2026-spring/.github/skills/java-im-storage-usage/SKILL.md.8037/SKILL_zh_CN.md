...
| 规约 | 处理方式 |
|------|---------|
| `.github/instructions/java-naming.instructions.md` | 🟢 **必读** — 包名、类名、方法名、变量命名 |
| `.github/instructions/java-code-style.instructions.md` | 🟢 **必读** — `final` 局部变量、`try-with-resources`、字符串字面量等 |
...
- **防范路径穿越攻击。** 不要将用户输入直接用作文件名、路径。对包含 `..` 或 `/`、`\` 的输入进行清洗或拒绝（若项目另有 Java 专用安全规约则参考之；若没有，则将 `.github/instructions/jssp-security.instructions.md` 的思路套用到 Java 的异常机制中）
...
- 异常可以直接以 `throws IOException` 向外传播，也可以用业务异常包装。错误消息应按照 `.github/instructions/java-javadoc.instructions.md` 的规约用日文进行说明性描述
