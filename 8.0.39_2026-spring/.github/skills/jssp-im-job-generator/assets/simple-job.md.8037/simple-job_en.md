...
Job parameters are declared with the `@parameter` annotation at the top of the function and retrieved by key via `Contexts.getJobSchedulerContext().getParameter()`. `execute()` takes no arguments.
...
> ⚠️ Receiving via an argument like `function execute(params)` and calling `JSON.parse()` is incorrect. Always retrieve with `getParameter()`.
 ...
 * Called by the job scheduler (takes no arguments).
 *
