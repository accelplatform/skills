...
使用 IM-共通主数据的 `imACMSearch` 调用私有组搜索对话框的实现示例。
        ...
        └── form.imds-form
            └── section
                └── imds-field-container
                    └── imds-field（私有组名称）
                        ├── input[type="hidden"]    ... 私有组代码（隐藏字段）
                        └── imds-textbox-control    ... 私有组名称（带搜索图标・只读）
```
...
**要点：**
- 在 `tabs` 中指定私有组搜索的插件 ID `jp.co.intra_mart.master.app.search.tabs.private_group.list`
- 私有组搜索只有关键词搜索标签页（没有树形搜索标签页）
- 用 `prop` 指定传递给回调函数的获取项目（`private_group_cd`、`private_group_name`）
- `type: 'single'` 为单选模式，`'multiple'` 为多选模式
- `wnd_close: true` 表示选择后自动关闭对话框

### 1.3 回调函数

接收搜索对话框中选择的私有组信息，并将其反映到表单字段中。
...
- 私有组的主键由 `private_grp_cd` 这一个字段构成
- 私有组搜索的 `data` 包含 `private_group_cd`、`user_cd`、`private_group_name`
...
<div class="imds-field is-horizontal imds-w-15 sample-private-group">
  <div class="imds-field-label"><label for=":privateGroupName:">私有组名称</label></div>
  <div class="imds-field-control">
    <div class="imds-textbox-control">
      <input type="hidden" id=":privateGroupCode:" value="">
      <input type="text" id=":privateGroupName:" placeholder="请选择私有组名称" class="imds-textbox" readonly value="">
...
**要点：**
- 使用 `input[type="hidden"]` 保存私有组代码，用于服务器提交
...
- `:privateGroupCode:`、`:privateGroupName:` 为占位符，实现时请替换为唯一 ID
...
- 多选模式下，将回调的 `result` 保存到变量中，在重新打开对话框时作为 `default_selected` 参数传递，可恢复对话框上已选择的项目。实现模式如下：
  - 声明保持变量：`let selectedPrivateGroup = [];`
  - 在回调内保存：`selectedPrivateGroup = result;`
  - 启动对话框时传递：`default_selected: selectedPrivateGroup`
- 私有组仅显示登录用户自身所属的组
