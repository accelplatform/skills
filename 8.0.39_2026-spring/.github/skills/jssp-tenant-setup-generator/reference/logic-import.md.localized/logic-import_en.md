...
This is an exception to the "no direct Java access" principle described in `{{AGENT_RULES}}/jssp-security.instructions.md`. **It is strictly forbidden in business SSJS** and is **only permitted in extended import JS for tenant environment setup**. Reasons:
