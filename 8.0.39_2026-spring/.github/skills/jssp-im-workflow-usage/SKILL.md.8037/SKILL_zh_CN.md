...
description: 新建 IM-Workflow 集成程序。提供动作处理（申请·审批·否决·退回）、案件开始/终了处理、分支条件处理、申请画面、审批画面、确认画面的实现模式。当提及工作流、申请画面、审批画面、确认画面、动作处理、案件处理、分支条件、审批流程时使用。作业调度器的定期批处理请使用 jssp-im-job-generator。
...
本技能生成申请画面・审批画面（`.html` + `.js`）以及动作处理（`.js`）。全局视图请参阅 `.github/instructions/README.md` 中的「规约文件一览（一行摘要 + 适用范围标签）」。本技能特有的重要度：
...
| 生成对象 | 模板 | 存放位置 |
|---------|------|---------|
| 申请画面（申请＋临时保存）— workflowOpenPage 方式（.html / .js） | `assets/simple-apply-screen.md` | `{功能名}/workflow/apply/` |
| 申请画面（申请＋临时保存）— 处理模态框方式 ★推荐（.html / .js） | `assets/modal-apply-screen.md` | `{功能名}/workflow/apply/` |
| 审批画面（处理画面）— workflowOpenPage 方式（.html / .js） | `assets/simple-approve-screen.md` | `{功能名}/workflow/approve/` |
| 审批画面（处理画面）— 处理模态框方式 ★推荐（.html / .js） | `assets/modal-approve-screen.md` | `{功能名}/workflow/approve/` |
| 确认画面 — workflowOpenPage 方式（.html / .js） | `assets/simple-confirm-screen.md` | `{功能名}/workflow/confirm/` |
| 确认画面 — 处理模态框方式 ★推荐（.html / .js） | `assets/modal-confirm-screen.md` | `{功能名}/workflow/confirm/` |
...
- `reference/api-im-workflow-modal.md` — 处理模态框 API（`imWorkflow.modal.showApply()`）参考
- `reference/api-im-workflow-modal-tempsave.md` — 处理模态框 API（`imWorkflow.modal.showTemporarySave()`）参考
- `reference/api-im-workflow-modal-process.md` — 处理模态框 API（`imWorkflow.modal.showProcess()`）参考
...
生成申请画面时，**应首先让用户选择方式**。若无法判断采用哪种，推荐「处理模态框方式」。

### 比较表

| 观点 | workflowOpenPage 方式 | 处理模态框方式（★推荐） |
|------|----------------------|------------------------|
| 实现的简洁性 | 略复杂（需要 11 个 `$imw*` 变量） | 简洁（无需绑定变量） |
| 路由 XML | **不需要**（从内容定义直接调用） | **需要**（创建 `routing-jssp-config/*.xml`） |
| 临时保存按钮 | 与申请配置在同一画面（`workflowOpenPage('1')`） | 与申请配置在同一画面（`showTemporarySave()`） |
| 返回按钮 | 需要（与 `imw-back-form` 表单联动） | 需要（使用 `imWorkflow.transition.returnTo()`） |
| 申请按钮的处理器 | 调用 `workflowOpenPage('0')`（同步） | `async/await imWorkflow.modal.showApply(...)` |
| 临时保存按钮的处理器 | 调用 `workflowOpenPage('1')`（同步） | `async/await imWorkflow.modal.showTemporarySave(...)` |
| 模板 | `assets/simple-apply-screen.md` | `assets/modal-apply-screen.md` |
| 参考 | `reference/imart-tag-workflow-open-page.md` | `reference/api-im-workflow-modal.md` / `reference/api-im-workflow-modal-tempsave.md` |

### 推荐理由

处理模态框方式无需绑定变量（11 个 `$imw*`），代码更简洁。临时保存・返回按钮两种方式都需配置，但在模态框方式下可用 `showTemporarySave()`（userDataId 由内部自动编号）实现临时保存、用 `imWorkflow.transition.returnTo()` 实现返回，写法简洁、易于维护（无需像 workflowOpenPage 方式那样编写 `imw-back-form` 表单或手动为 userDataId 编号）。由于成为可从 URL 直接访问的独立画面，也更易于应对来自门户或链接的调用。

### 向用户确认的措辞（示例）

> 请选择申请画面的实现方式。
>
> 1. **处理模态框方式**（推荐）— 使用 `imWorkflow.modal.showApply()`。无需绑定变量，代码简洁。临时保存・返回也可用 `showTemporarySave()` / `transition.returnTo()` 简洁实现。需要路由 XML。
> 2. **workflowOpenPage 方式** — 使用 `<imart type="workflowOpenPage">`。从内容定义直接调用的形式。无需路由 XML。
>
> 如难以抉择，推荐 **处理模态框方式**。

### 审批画面・确认画面的方式选择

审批画面（处理画面）・确认画面与申请画面一样，也有 **workflowOpenPage 方式（旧型）** 与 **处理模态框方式（新型・★推荐）** 两种方式。方式选择的思路与申请画面相同（模态框方式无需绑定变量、代码简洁；需要路由 XML）。模板请参照以下内容。

| 画面 | workflowOpenPage 方式（旧型） | 处理模态框方式（新型・★推荐） |
|------|------------------------------|------------------------------|
| 审批画面（处理画面） | `assets/simple-approve-screen.md`（`workflowOpenPage('4')`） | `assets/modal-approve-screen.md`（`showProcess()`） |
| 确认画面 | `assets/simple-confirm-screen.md`（`workflowOpenPage('5')`） | `assets/modal-confirm-screen.md`（`showConfirm()`） |

- **确认画面与详情画面（处理详情・参照详情）是不同的事物**：详情画面是仅查看信息即结束的只读画面，而确认画面与处理画面一样执行「确认」操作（`workflowOpenPage('5')` / `showConfirm()`）。确认画面须配置确认操作的提交按钮。
- **申请内容的复原**：审批画面・确认画面针对未完成案件，不会传入 `imwUserDataId`，因此使用 `ActvMatter(systemMatterId).getMatterPropertyList()`（不使用 `UserActvMatterPropertyValue(userDataId)`）。
...
1. 向用户确认需求（处理类型、功能名、业务逻辑内容）
   - **包含申请画面时**：向用户出示上述「申请画面的方式选择」章节的确认措辞，确定方式后再进入实现
2. 参考对应的 assets 模板进行生成
3. 确认文件存放位置（`src/main/jssp/src/{功能名}/workflow/` 目录下）

**注意（路由 XML）：**
- **workflowOpenPage 方式**：因内容定义中直接指定 JSSP 路径，路由表（XML）**不需要**。
- **处理模态框方式**：因是从 URL 直接访问的独立画面，路由表（XML）**需要**。须在 `routing-jssp-config/` 目录下创建 XML（`assets/modal-apply-screen.md` 中附有示例）。
...
- 表名·列名须与动作处理的 SQL 一致
- **列的类型须遵循 `.github/skills/jssp-page-generator/reference/ddl-type-mapping.md` 的类型映射表**（不得凭记忆或推测书写类型名）
...
> **注记**：IM-Workflow 画面由工作流引擎通过 XML 的 `scriptPath` 直接调用，不经过路由表。因此适用于 **与 `.github/instructions/jssp-file-structure.instructions.md` 的 `view/{view}.js`（每个画面的 snake_case 唯一名）规约不同的另一套规约**——这源于调用方不同。请按本章节的规约处理（`.github/instructions/jssp-file-structure.instructions.md` 中也以「不经过路由表调用的画面的例外规约」明确记载）。
...
- **展示页面（.html）须遵守 `.github/instructions/jssp-presentation-page.instructions.md` 的编码规范**。尤其是验证实现的规范遵守是必须的。生成完成后，使用 `reference/screen-generation-checklist.md` 进行自检。
...
```bash
node .github/skills/jssp-im-workflow-usage/scripts/validate-workflow-code.js src/main/jssp/src/{功能名}/
```
...
执行 `.github/skills/jssp-page-generator/reference/post-generation-verification.md` 的所有步骤。
以下是过去曾发生问题的重点项目：
...
申请画面·审批画面·详情画面·确认画面等 **生成了 `.html` 时必须执行**。虽然 `validate-workflow-code.js` 能检测出诸如 `imds-selectbox` 之类的已知错误类名，但 **它无法覆盖与 reference 的完整对照**，因此须打开 `.github/skills/jssp-imds-theme/reference/` 进行目视再确认。

#### 实施步骤

1. 扫描已生成的各 `.html`，列出使用的 imds 组件（文本框、文本区域、下拉框、复选框、单选框、按钮、表格、对话框、字段、标签页、日历输入、横幅/行内消息等）
2. 针对每个组件，**使用 Read 工具打开 `.github/skills/jssp-imds-theme/reference/imds-html-{component}.md` 进行再确认**（不依赖记忆）
...
#### 与 `<imart type="workflowOpenPage">` 相关的追加检查（仅 workflowOpenPage 方式）
...
#### 处理模态框方式特有的检查（仅处理模态框方式）

> **workflowOpenPage 方式时可跳过本章节。**

- `<head>` 中是否记述了 `<meta http-equiv="X-Intramart-Secure-Token" content="<imart type="imSecureToken" mode="value" />"/>`？（`name="im_secure_token"` 形式无法工作）
- `<head>` 中是否记述了 `<script src="im_workflow/js/api_base.js" defer></script>`？（`defer` 必需）
- 申请按钮的事件监听器是否为 `async` 函数？（`await imWorkflow.modal.showApply(...)` 所必需）
- 是否残留了 `workflowOpenPage('0')` 等 pageType 调用？（处理模态框方式不使用）
- 是否残留了 `$imwFlowId` / `$imwMatterName` 等 `$imw*` 绑定变量？（处理模态框方式不需要）
- 临时保存按钮・返回按钮是否已删除？（处理模态框方式不需要）
- 路由 XML（`routing-jssp-config/*.xml`）是否已创建？
- `flowId` 中是否设置了实际的流程 ID，或是否残留了 TODO 注释？（确认是否为正确的值，而非临时值）
- 临时保存・处理（审批/否决）・确认画面时，`imWorkflow.transition.afterProcess()` 是否在 `then()` 或 `await` 之后调用？（完成・取消两种情况下都需要调用）
...
| 1 | `tempSave` | **`{功能名}/workflow/apply/`（与申请画面共用）** | 临时保存画面（申请画面兼作临时保存） |
| 2 | `applyTask` | `{功能名}/workflow/apply_task/`（或与 apply 共用） | 申请（起票案件）画面 |
