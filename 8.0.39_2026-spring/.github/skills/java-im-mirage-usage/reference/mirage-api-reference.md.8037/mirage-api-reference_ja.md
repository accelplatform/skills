...
未指定時は `NameConverter` によりクラス名から自動変換されるが、本プロジェクトでは明示指定を必須とする（`.github/instructions/java-entity.instructions.md` 参照）。
...
`.github/instructions/java-entity.instructions.md` により、本プロジェクトでは `GenerationType.APPLICATION` 以外の使用を禁止している。
