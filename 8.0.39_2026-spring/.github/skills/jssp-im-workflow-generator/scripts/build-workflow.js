  ...
  for (const locale of locales) {
    ...
    for (const an of actionNodes) {
      ...
      if (!ctx._contentsPluginIds[an.fullId]) {
        ctx._contentsPluginIds[an.fullId] = `cp_${short}_${randomId(8)}`;
      }
      ...
      xml.tagStr('pluginId', 'jp.co.intra_mart.workflow.plugin.event.node.action.process.pluginScriptExecutor');
    ...
    if (spec.matterEndProcess) {
      ...
      const mepExPoint = noTran
        ? 'jp.co.intra_mart.workflow.plugin.event.flow.matter.end.process.no.transaction'
        : 'jp.co.intra_mart.workflow.plugin.event.flow.matter.end.process';
      const mepPluginId = mepExPoint + '.pluginScriptExecutor';
      if (!ctx._matterEndPluginId) {
        ctx._matterEndPluginId = `cp_${short}_mep_${randomId(5)}`;
      }
      xml.openObj();
