...
**検索結果テンプレートに `routing-jssp-config/` のルーティング設定は不要**。テンプレートは IM-ContentsSearch が `<template-path>` を介して直接呼び出すため、通常の画面のような URL ルーティングを経由しない（詳細は `.claude/rules/jssp-file-structure.md` の「ルーティングテーブル経由で呼ばれない画面の例外規約」参照）。
