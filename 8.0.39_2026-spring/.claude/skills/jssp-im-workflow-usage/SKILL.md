...
本スキルは申請画面・承認画面（`.html` + `.js`）およびアクション処理（`.js`）を生成する。全体像は `rules/README.md` の「規約ファイル一覧（一行要約 + 適用範囲タグ）」を参照。本スキル特有の重要度:
...
> **注記**: IM-Workflow の画面はワークフローエンジンが XML の `scriptPath` から直接呼び出すため、ルーティングテーブルを介さない。よって `rules/jssp-file-structure.md` の `view/{view}.js`（snake_case の画面ごとの固有名）規約とは **呼び出し元が異なる別系統の規約** が適用される。本セクションの規約に従うこと（`rules/jssp-file-structure.md` 側にも「ルーティングテーブル経由で呼ばれない画面の例外規約」として明記済み）。
...
- **プレゼンテーションページ（.html）は `rules/jssp-presentation-page.md` のコーディング規約に従うこと**。特にバリデーション実装は規約準拠が必須。生成完了後、`reference/screen-generation-checklist.md` でセルフチェックを行うこと
