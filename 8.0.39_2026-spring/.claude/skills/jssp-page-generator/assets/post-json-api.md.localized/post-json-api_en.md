...
- Follow `{{AGENT_RULES}}/jssp-error-handling.md` for error code naming.
...
Declaring `@typedef {Error & {code, httpStatus}} ApiError` per file triggers `tsc` `TS2300 Duplicate identifier`. **Casting with an inline `@type` annotation is the canonical form of this template.** See `{{AGENT_RULES}}/jssp-error-handling.md` for details.
...
- `{{AGENT_RULES}}/jssp-error-handling.md` - Error code naming and API error response format
- `{{AGENT_RULES}}/jssp-security.md` - Overall input validation policy
