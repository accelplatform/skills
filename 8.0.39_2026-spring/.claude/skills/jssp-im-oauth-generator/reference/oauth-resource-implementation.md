...
- ログ出力時も同様（`rules/jssp-logging.md` の `MaskUtil` を活用）
...
- `rules/jssp-error-handling.md` - エラーレスポンス形式・HTTP ステータス
- `rules/jssp-security.md` - SQL インジェクション・XSS 対策（OAuth API でも同様に適用）
...
- [ ] SQL アクセスがある場合は `DbParameter` でバインドしているか（文字列連結禁止、`rules/jssp-2way-sql.md` 参照）
