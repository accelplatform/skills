...
Web API Maker 的官方文档・javadoc 习惯上将持有 `@Path`/HTTP 方法注解的类称为「服务类」，但这是 Web API Maker 自身的术语。**依照本项目的命名规则（`.agents/requirements/java-naming/AGENTS.md` 中「REST API 端点 → `Endpoint` 后缀」），承担此角色的 Java 类应添加 `Endpoint` 后缀**（应为 `OrderEndpoint` 而非 `OrderService`）。
...
2. 设计包・工厂类名・Endpoint 类名（遵循 `.agents/requirements/java-naming/AGENTS.md`。Endpoint 类为 `Xxx` + `Endpoint`，工厂类为 `Xxx` + `EndpointFactory`）
...
7. 确认是否遵循 `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
...
11. 是否遵循 `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
