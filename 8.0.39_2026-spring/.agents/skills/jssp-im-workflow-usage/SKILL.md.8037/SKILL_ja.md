...
- アクション処理・案件処理・分岐条件処理はすべてファンクションコンテナであるため、コーディング規約（`let` の使用、命名規則等）に従うこと。詳細は `.agents/skills/jssp-page-generator/reference/` 配下を参照
- テンプレートは必要に応じてカスタマイズ
...
- **プレゼンテーションページ（.html）は `.agents/requirements/jssp-presentation-page/AGENTS.md` のコーディング規約に従うこと**。特にバリデーション実装は規約準拠が必須。生成完了後、`.agents/skills/jssp-im-workflow-usage/reference/screen-generation-checklist.md` でセルフチェックを行うこと
