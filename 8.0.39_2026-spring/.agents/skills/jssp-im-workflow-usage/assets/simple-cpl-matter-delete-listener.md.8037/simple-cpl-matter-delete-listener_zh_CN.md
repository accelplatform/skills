...
IM-Workflow 已完成案件删除处理监听器程序的模板。
无需画面，在已完成案件删除时自动执行的批处理类处理。
...
src/main/jssp/src/{功能名}/workflow/
  └── cpl_matter_delete_listener.js    # 已完成案件删除监听器
```
...
## 已完成案件删除监听器（cpl_matter_delete_listener.js）
...
function execute(loginGroupId, localeId, systemMatterId, userDataId) {
    ...
    try {
        ...
        logger.error('[cplMatterDelete] 发生错误。systemMatterId={}, error={}', systemMatterId, e.message);
...
- **已完成案件删除监听器**：[assets/simple-cpl-matter-delete-listener.md](assets/simple-cpl-matter-delete-listener.md)
  - 已完成案件删除时执行的监听器处理
...
当用户请求"创建工作流已完成案件删除监听器"时，参考此 assets 中的代码，适当定制后生成。
