...
- [ ] [jssp-testing](../../../requirements/jssp-testing/AGENTS.md) has been read and understood
...
3. Design test cases based on the test perspectives in `.agents/requirements/jssp-testing/AGENTS.md`
4. **Before generating test code**, ask the user the following 2 questions (do not generate code first)
   ...
   **Question ①: Screenshots (layout inspection)**
   > "Would you like to include screenshots using `takeScreenshot()`?
   > This lets the coding agent load the images after test execution to visually verify layout issues."

   **Question ②: Visual regression tests (regression detection)**
   > "Would you like to add visual regression tests using `toHaveScreenshot()`?
   > The first run generates baseline images; subsequent runs automatically detect visual changes."

5. Generate the test code after receiving the user's answers (refer to example code under `assets/`)
...
| `.agents/requirements/jssp-testing/AGENTS.md` | Test perspectives and configuration conventions |
...
Refer to the "Screenshots" section of `assets/test-helpers.md` for helper function implementations.

### Layout Inspection (`takeScreenshot`)

Used for the coding agent to visually inspect for layout issues by reading screenshots after test execution.
**Insert `takeScreenshot()` at the following points only if the user answered "yes" to Question ① in the test generation steps.**

| Timing | Example |
|--------|---------|
| After page load completes | Immediately after `waitForSelector` |
| After opening/closing a dialog | Immediately after confirming the `is-active` class |
| After CRUD operations | After registration, update, or deletion result is reflected |
| After validation errors are displayed | Immediately after confirming the error class is applied |

Output path: `test-results/screenshots/<name>.png`

### Evidence Recording (`screenshotStep`)

Use only when the instruction includes keywords such as "leave evidence", "save screenshots", or "evidence".
Replace the normal `takeScreenshot()` with `screenshotStep()` to record each test step with `fullPage: true`.

Output path: `test-results/evidence/<testName>_<label>.png`

### Visual Regression (`toHaveScreenshot`)

**Add a visual regression `test.describe` block only if the user answered "yes" to Question ② in the test generation steps.**
Refer to the "Visual Regression" section of `assets/test-helpers.md` for the implementation pattern.

If the user did not answer, it is acceptable to ask Question ② again. Do not decide on your own whether to include it.

## Notes

- Refer to `.agents/requirements/jssp-testing/AGENTS.md` for details on test perspectives
