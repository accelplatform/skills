...
- エラーコード命名は `.agents/requirements/jssp-error-handling/AGENTS.md` に従う
...
`@typedef {Error & {code, httpStatus}} ApiError` をファイル単位で宣言すると `tsc` の `TS2300 Duplicate identifier` を引き起こす。**インライン `@type` 注釈でキャストするのが本テンプレートの標準形**。詳細は `.agents/requirements/jssp-error-handling/AGENTS.md` を参照。
...
- `.agents/requirements/jssp-error-handling/AGENTS.md` - エラーコード命名規則と API エラーレスポンス形式
- `.agents/requirements/jssp-security/AGENTS.md` - 入力検証の全体方針
