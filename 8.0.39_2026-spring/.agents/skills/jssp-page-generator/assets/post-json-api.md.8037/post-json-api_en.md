...
- Follow `.agents/requirements/jssp-error-handling/AGENTS.md` for error code naming.
...
Declaring `@typedef {Error & {code, httpStatus}} ApiError` per file triggers `tsc` `TS2300 Duplicate identifier`. **Casting with an inline `@type` annotation is the canonical form of this template.** See `.agents/requirements/jssp-error-handling/AGENTS.md` for details.
...
- `.agents/requirements/jssp-error-handling/AGENTS.md` - Error code naming and API error response format
- `.agents/requirements/jssp-security/AGENTS.md` - Overall input validation policy
