...
- `.agents/requirements/jssp-function-container/AGENTS.md` - 基本構造
- `.agents/requirements/jssp-error-handling/AGENTS.md` - エラーレスポンス・HTTP ステータス
- `.agents/requirements/jssp-logging/AGENTS.md` - ログ出力
- `.agents/requirements/jssp-security/AGENTS.md` - SQL インジェクション・機密情報の取扱い
- `.agents/requirements/jssp-2way-sql/AGENTS.md` - DB アクセスがある場合
- `.agents/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()`
- `.agents/skills/jssp-page-generator/reference/api-account-context.md` - 認証ユーザ取得
...
```bash
node .agents/skills/jssp-im-oauth-generator/scripts/build-oauth.js \
     /tmp/{機能名}.spec.json
```
...
**補完時の規約:**
- DB アクセスを伴う場合は `.agents/requirements/jssp-2way-sql/AGENTS.md` に従い、`src/main/jssp/src/{機能名}/sql/` 配下に SQL ファイルを外部化し `executeByTemplate` を使う
