...
> For details, see `.agents/skills/jssp-page-generator/reference/api-account-context.md`.
...
For implementation patterns when receiving `application/json` body, see `.agents/skills/jssp-page-generator/assets/post-json-api.md` (the body retrieval procedure is the same in the OAuth version).
...
- Same when outputting logs (use `MaskUtil` in `.agents/requirements/jssp-logging/AGENTS.md`)
...
- `.agents/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()` details
- `.agents/skills/jssp-page-generator/reference/api-account-context.md` - Authenticated user context
- `.agents/skills/jssp-page-generator/reference/argument-request.md` - request argument
- `.agents/requirements/jssp-error-handling/AGENTS.md` - Error response format / HTTP status
- `.agents/requirements/jssp-security/AGENTS.md` - SQL injection / XSS countermeasures (applied similarly in OAuth API)
...
- [ ] When using SQL access, is `DbParameter` used for binding (string concatenation forbidden, see `.agents/requirements/jssp-2way-sql/AGENTS.md`)?
