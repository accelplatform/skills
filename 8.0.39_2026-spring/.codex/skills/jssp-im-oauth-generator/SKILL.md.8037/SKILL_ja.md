...
- `{{AGENT_ROOT}}/requirements/jssp-function-container.md` - 基本構造
- `{{AGENT_ROOT}}/requirements/jssp-error-handling.md` - エラーレスポンス・HTTP ステータス
- `{{AGENT_ROOT}}/requirements/jssp-logging.md` - ログ出力
- `{{AGENT_ROOT}}/requirements/jssp-security.md` - SQL インジェクション・機密情報の取扱い
- `{{AGENT_ROOT}}/requirements/jssp-2way-sql.md` - DB アクセスがある場合
...
**補完時の規約:**
- DB アクセスを伴う場合は `{{AGENT_ROOT}}/requirements/jssp-2way-sql.md` に従い、`src/main/jssp/src/{機能名}/sql/` 配下に SQL ファイルを外部化し `executeByTemplate` を使う
