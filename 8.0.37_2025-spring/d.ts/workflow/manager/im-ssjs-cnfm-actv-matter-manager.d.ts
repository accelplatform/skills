/**
 * 確認（未完了案件）マネージャオブジェクト。
 *
 * コンストラクタに指定したシステム案件ID とノードID で取得した、ある案件の確認ノードに対して、
 * あるユーザが処理権限があるかのチェックや、処理できる組織情報の取得、
 * また指定した確認ノードの確認処理を行うことができます。
 *
 * 本オブジェクトでは、未完了状態の案件に対しての確認処理を行います。
 * 完了案件に対して、確認関連処理を行うには CnfmCplMatterManager を利用してください。
 *
 * コンストラクタに指定したロケールID は getAuthUserOrgz(String) で利用されます。
 * 他のメソッドでは利用されません。
 *
 * @since 8.0.37 (2025 Spring)
 * @see https://api.intra-mart.jp/iap/apilist-ssjs/doc/im_workflow/CnfmActvMatterManager/index.html
 */
declare class CnfmActvMatterManager {
  /**
   * 確認（未完了案件）マネージャのインスタンスを生成します。
   ...
   *
   * 以下の２つの要件を満たす時、確認可能（true）を返却します。
   * ・確認ノードが処理可能な状態か
