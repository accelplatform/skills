/**
 * ユーザデータ案件プロパティ情報（完了案件）マネージャオブジェクト。
 *
 * ユーザデータ案件プロパティ情報（完了案件）の新規登録、更新、削除を行います。
 * 本オブジェクトではユーザデータID をキーで検索や更新処理を行います。
 * 検索や更新処理の対象になるテーブルは完了案件関連テーブルである「imw_t_cpl_matter_user_data」です。
 *
 * 処理中の未完了案件や過去案件のユーザデータプロパティ情報の操作には
 * UserActvMatterPropertyValue、UserArcMatterPropertyValue を利用してください。
 *
 * システム案件ID をキーで完了案件情報からユーザデータ案件プロパティ情報を取得するには、
 * CplMatter を利用してください。
 *
 * @since 8.0.37 (2025 Spring)
 * @see https://api.intra-mart.jp/iap/apilist-ssjs/doc/im_workflow/UserCplMatterPropertyValue/index.html
 */
declare class UserCplMatterPropertyValue {
  /**
   * ユーザデータ案件プロパティ情報（完了案件）マネージャのインスタンスを生成します。
   */
  constructor();
   ...
   *
   * 「imw_t_cpl_matter_user_data」テーブルに登録します。
   ...
   *
   * ユーザデータID と案件プロパティキーで「imw_t_cpl_matter_user_data」テーブルから取得します。
   *
   ...
   *
   * ユーザデータID で「imw_t_cpl_matter_user_data」テーブルから取得します。
   *
   * @param userDataId ユーザデータID
