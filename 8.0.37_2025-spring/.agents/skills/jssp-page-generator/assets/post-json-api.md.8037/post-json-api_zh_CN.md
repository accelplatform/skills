...
- 错误代码命名遵循 `.agents/requirements/jssp-error-handling/AGENTS.md`
...
按文件单位声明 `@typedef {Error & {code, httpStatus}} ApiError` 会引发 `tsc` 的 `TS2300 Duplicate identifier`。**用内联 `@type` 注解进行类型转换是本模板的标准形式**。详情参见 `.agents/requirements/jssp-error-handling/AGENTS.md`。
...
- `.agents/requirements/jssp-error-handling/AGENTS.md` - 错误代码命名规则与 API 错误响应格式
- `.agents/requirements/jssp-security/AGENTS.md` - 输入验证的整体方针
