...
`.agents/requirements` 配下には `IOException` のラップ方針を定めた Java 向け専用規約は存在しない（2026年時点）。`get()` 使用時の例外処理は `assets/identifier-basic-usage.md` のパターンに従う。
...
4. `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md` に準拠しているか
