...
- [ ] 在路由（`routing-jssp-config` 的 `file-mapping`）或 OAuth（`oauth-client-resources-config` 的 `client-resource`）中通过 `<authz uri="service://..." action="execute" />` 指定的 URI，已注册到 `authz-resource.xml` 的 `uri`。由于不使用 `welcome-all`，每个画面/API 都需要对应的授权资源
- [ ] 租户管理员（`tenant_manager`）由 `build-setup-import.js` 自动授予所有 service 资源和所有菜单组的 PERMIT（无需在 `authzPolicies` 中列出）。**其他目标角色／用户**按设计书或提示的指示记述在 `authzPolicies` 中
...
## IM-Workflow 逻辑流插件注册（组合使用 `extendsImport` + WF/LD 联动时）

- [ ] `import-<artifactId>-config-<N>.xml` 中 `<extends-import>` 内的 `<extends-import-class>` 按以下**顺序**排列（构建脚本会以相反顺序生成，因此**生成后需手动重新排序**）：
  1. `<key>_logic_import.js` — 导入 IM-LogicDesigner 流程 ZIP（最先）
  2. `<key>_workflow_import.js` — 导入 IM-Workflow 定义 XML（第二）
  3. `<key>_import.js` — 将 LD 流程注册为 IM-Workflow 处理对象插件（最后）
- [ ] `<key>_import.js` 中包含对 `WorkflowLogicFlowManager.createLogicFlow()` 的调用
- [ ] `IMW_LOGIC_FLOW_PLUGINS` 数组中的 `logicFlowId` 与 IM-LogicDesigner 中注册的流程 ID 一致
- [ ] 注册的 `resourceTypes` 与需求相符（`authority_process` / `authority_confirm` / `authority_matter_handle`）
- [ ] 执行租户环境安装后，在 IM-Workflow 管理画面的"逻辑流列表"中可以看到目标流程已注册
