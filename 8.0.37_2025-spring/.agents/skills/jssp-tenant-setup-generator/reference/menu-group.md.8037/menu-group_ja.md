...
> **テナント管理者（`tenant_manager`）は `build-setup-import.js` が全メニューグループに自動付与する**ため、`authzPolicies` に明示する必要はない。下記の例のように、**それ以外の対象ロール／ユーザ**（設計書・プロンプトで指示されたもの）のみを記述する。詳細は [authz-policy.md](authz-policy.md) の「既定ポリシー」を参照。
