...
| 授权 | `<authz uri="service://sample_oauth/get_user" action="execute" />`（通过授权资源控制；与 scope 以 AND 评估） |
...
node .agents/skills/jssp-im-oauth-generator/scripts/build-oauth.js \
     .agents/skills/jssp-im-oauth-generator/examples/sample_oauth.spec.json
```
