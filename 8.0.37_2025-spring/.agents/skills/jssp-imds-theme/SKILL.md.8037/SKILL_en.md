...
When implementing input forms, always follow the canonical pattern in `.agents/skills/jssp-page-generator/assets/simple-form.md`.
...
```bash
node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <target file or directory>
```
...
| Undefined class detection | Classes with `imds-*` prefix that are not in `reference/` or `.agents/requirements/` are reported as `IMDS-U-001` (warning). Helps catch typos or imaginary classes (e.g., `imds-page-header`) |
...
5. Run `node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <generated file>` and confirm PASS
...
4. Run `node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <generated file>` and confirm PASS
