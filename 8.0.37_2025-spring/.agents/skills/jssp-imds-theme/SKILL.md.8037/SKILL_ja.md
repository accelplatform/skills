...
入力フォームを実装する際は、必ず `.agents/skills/jssp-page-generator/assets/simple-form.md` の正規パターンに従う。
...
```bash
node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <対象ファイルまたはディレクトリ>
```
...
| 未定義クラス検出 | `imds-*` プレフィックスを持つが `reference/` ・`.agents/requirements/` いずれにも記載が無いクラスを `IMDS-U-001`（warning）として通知。typo / 架空クラス（例: `imds-page-header`）を発見できる |
...
5. `node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <生成ファイル>` を実行し PASS を確認
...
4. `node .agents/skills/jssp-imds-theme/scripts/validate-imds.js <生成ファイル>` を実行し PASS を確認
