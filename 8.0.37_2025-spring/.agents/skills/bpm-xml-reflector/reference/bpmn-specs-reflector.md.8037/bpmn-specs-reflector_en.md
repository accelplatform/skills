...
**This step must always be executed via `reflector.reflect()` in `.agents/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js` (which internally calls `reflectProcessIdReplacements()`).
...
The reflection logic is implemented in `.agents/skills/bpm-xml-reflector/scripts/bpmn-specs-reflector.js`. The following is its overview and how to call it.
