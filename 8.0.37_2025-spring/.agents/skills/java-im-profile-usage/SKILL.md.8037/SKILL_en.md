...
No dedicated Java convention defining exception-handling policy exists under `.agents/requirements` (as of 2026). Follow the business-exception-wrapping pattern in `assets/profile-basic-usage.md` for `UserProfileImageManager`'s exceptions (`BizApiException`, a checked exception).
...
6. Whether the code complies with `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
