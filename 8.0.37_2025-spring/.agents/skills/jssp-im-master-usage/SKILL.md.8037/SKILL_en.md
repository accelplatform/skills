...
This skill generates HTML snippets (with `imACMSearch` calls) that get embedded into screens. It does not generate `.js` on its own, so the relevant conventions are HTML-centric. See `.agents/requirements/README.md` for the full picture.
