...
本スキルはバッチ処理（`.js` のみ、画面なし）を生成する。全体像は `.agents/requirements/README.md` 参照。
...
ジョブパラメータは、関数先頭の **`@parameter` JSDoc アノテーション**で宣言し、`Contexts.getJobSchedulerContext().getParameter()` で**キーごとに個別取得**する。`execute()` は**引数を取らない**。
...
- `@parameter <パラメータ名> <デフォルト値>` の形式で、ジョブで受け取るパラメータを宣言する
- 宣言した値は `getParameter('<パラメータ名>')` で**文字列として**取得する（未設定時は `null`）
- 複数パラメータは `@parameter` を複数行記述し、それぞれ `getParameter()` で取得する
- ⚠️ `function execute(params)` のように**引数で受け取って `JSON.parse()` する方式は誤り**。使用しないこと
- ⚠️ **JSDoc コメントの説明文（地の文）の中に `@parameter` という文字列を書かないこと**。ジョブスケジューラの解析器は行頭以外でも `@parameter` を検出し、直後の語をパラメータ名として誤って宣言してしまう（例: 「`@parameter` で宣言します」と書くと「で宣言します」という名前のパラメータが生成される）。説明したい場合は「アノテーション」等の別表現に言い換える
...
`.agents/skills/jssp-page-generator/reference/post-generation-verification.md` のステップ 1〜3 を実行する（ステップ 4 の画面検証はジョブには不要）。
