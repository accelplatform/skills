...
- **实体类应遵循 `.agents/requirements/java-entity/AGENTS.md` 的规约。** public 字段、无参构造函数、`GenerationType.APPLICATION` 等是该规约为本项目制定的规则，本技能只是在 im_mirage 的实现层面加以满足
...
- **继承 `AbstractDAO` 后，insert/update 时会自动设置审计字段（`createUserCd`/`createDate`/`recordUserCd`/`recordDate`）。** 无需手动设置，且不推荐手动设置（`.agents/requirements/java-entity/AGENTS.md` 中也有明确说明）
...
| 规约 | 处理方式 |
|------|---------|
| `.agents/requirements/java-entity/AGENTS.md` | 🟢 **必读** — 实体类的设计规约（public 字段・审计字段・类型对应表） |
...
| 在 JSSP（脚本开发模型）中使用 `TenantDatabase`/`SharedDatabase` API・2WaySQL（`executeByTemplate`） | `jssp-page-generator` + `.agents/requirements/jssp-2way-sql/AGENTS.md`（不在本技能范围内） |
...
2. 按照 `.agents/requirements/java-entity/AGENTS.md` 设计并实现实体类（`@Table`/`@Column`/`@PrimaryKey`、4 个审计字段）
...
7. 确认是否符合 `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
...
1. 实体类是否符合 `.agents/requirements/java-entity/AGENTS.md`（public 字段・无参构造函数・`GenerationType.APPLICATION`・4 个审计字段）
...
14. 是否符合 `.agents/requirements/java-naming/AGENTS.md` / `java-code-style.md` / `java-javadoc.md`
...
| 在 JSSP（脚本开发模型）中使用 `TenantDatabase`/`SharedDatabase` API・2WaySQL | `jssp-page-generator` + `.agents/requirements/jssp-2way-sql/AGENTS.md` |
