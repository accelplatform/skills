...
遵循 `.agents/requirements/java-entity/AGENTS.md` 的规约（public 字段、无参构造函数、4个审计项目字段）。
...
- `@PrimaryKey` 的 `generationType` 在本项目中仅使用 `GenerationType.APPLICATION`（参见 `.agents/requirements/java-entity/AGENTS.md`）
...
- 2WaySQL 的基本语法（`/*IF*/`/`/*BEGIN*/`/`/*param*/'dummy'`）与 JSSP 侧（`.agents/requirements/jssp-2way-sql/AGENTS.md`）通用。哑值的作用、LIKE 检索时的转义方针也可以采用相同的思路
