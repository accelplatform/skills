...
`.github/skills/jssp-tenant-setup-generator/reference/checklist.md` を参照する（読み替えは SKILL.md の「参照時の読み替え」）。
...
- [ ] `.github/instructions/jssp-code-style.instructions.md` に準拠している（変数宣言は `let`、`var` 不使用、文字列はシングルクォート）
