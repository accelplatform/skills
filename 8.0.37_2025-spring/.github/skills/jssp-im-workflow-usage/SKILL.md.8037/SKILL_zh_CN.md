...
| 规约 | 处理方式 |
|------|---------|
| `jssp-presentation-page.instructions.md` | 🟢 **必读** — 申请/审批画面的 HTML 结构、验证、id 命名 |
| `jssp-function-container.instructions.md` | 🟢 **必读** — `init()` 结构、workflowOpenPage 参数接收 |
| `jssp-naming.instructions.md` / `jssp-code-style.instructions.md` / `jssp-file-structure.instructions.md` | 🟢 必读 |
| `jssp-error-handling.instructions.md` / `jssp-security.instructions.md` | 🟢 必读 |
| `jssp-2way-sql.instructions.md` | 🔴 **画面侧原则上不需要**（申请/审批画面仅通过 `workflowOpenPage` 提交，无 DB 操作）。仅当动作处理侧使用 `db.executeByTemplate` 时参照 |
| `jssp-logging.instructions.md` | 🟡 仅在动作处理中实现日志时 |
