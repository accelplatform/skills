# ワークフロー 申請画面テンプレート（workflowOpenPage 方式 / 旧型）
...
> **モーダル方式（新型）との使い分け**: `imWorkflow.modal.showApply()` / `showTemporarySave()` を使う新型は `modal-apply-screen.md` を参照。新規開発では新型を推奨。本テンプレートは workflowOpenPage 方式（旧型）の申請画面が必要な場合に使用する。
...
function init(request) {
  ...
  // 一時保存のキーとなるユーザデータID。新規（request に無い）場合はアプリ側で採番する。
  // IM-Workflow 仕様: ユーザデータID は一時保存情報のキーのため、ユーザコンテンツ側で必ず採番すること。
  // （workflowOpenPage 方式には処理モーダル方式の showTemporarySave のような自動採番が無い）
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      function buildMatterName() {
        return '部品発注申請_' + document.getElementById(':partCode:').value;
      }

      // 業務データを送信用 hidden フィールドにコピーする
      // 旧方式（workflowOpenPage）では、表示用 input には name を付けず、
      // 送信直前に name 付き hidden フィールドへ値をコピーする（hidden の値がアクション処理の userParam として渡る）。
      // ※ 表示用 input に直接 name を付けると、hidden や select/radio と name が重複した際に
      //   userParam が配列（NativeArray）になりアクション処理でエラーになるため、必ず hidden 経由にする。
...
<imart type="workflowOpenPage"
  ...
  <!-- 業務データ送信用 hidden フィールド。表示用 input には name を付けず、
       送信直前に setUserDataToHidden() で値をコピーする（name 付きフィールドが userParam として渡る）。 -->
...
- **一時保存（`workflowOpenPage('1')`）を使う場合、userDataId をアプリ側で採番すること**。
  - IM-Workflow 仕様上、ユーザデータID は一時保存情報のキーであり、**ユーザコンテンツ側で必ず採番**する必要がある（[一時保存の仕様](https://document.intra-mart.jp/library/iap/public/im_workflow/im_workflow_specification/texts/detail_guide/projects/temporary_save/index.html)）。
  - 処理モーダル方式（`showTemporarySave`）は内部で自動採番するが、**workflowOpenPage 方式には自動採番が無い**ため、`init` で `request['imwUserDataId'] || Identifier.get()` のように採番する。
  - 採番が無いと、新規一時保存時にキーが空のまま保存され、申請画面で再表示しても入力内容が復元されない。
...
- **バリデーションの実装は `.github/instructions/jssp-presentation-page.instructions.md` の規約に従うこと**（以下は主要ルール）:
