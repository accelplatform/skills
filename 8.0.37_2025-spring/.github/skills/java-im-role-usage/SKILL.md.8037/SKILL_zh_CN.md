...
`.github/instructions` 下目前不存在规定异常处理方针的 Java 专用规约（截至2026年）。`RoleInfoManager` 的所有方法都被设计为抛出 `AdminException`（受检异常），业务异常的包装方针应遵循 `assets/role-basic-usage.md` 中的模式。
...
5. 确认是否遵循 `.github/instructions/java-naming.instructions.md` / `java-code-style.md` / `java-javadoc.md`
...
8. 是否遵循 `.github/instructions/java-naming.instructions.md` / `java-code-style.md` / `java-javadoc.md`
