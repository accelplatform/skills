...
**搜索结果模板不需要 `routing-jssp-config/` 下的路由配置。** 模板由 IM-ContentsSearch 通过 `<template-path>` 直接调用，因此不像普通画面那样经过 URL 路由（详见 `.github/instructions/jssp-file-structure.instructions.md` 中的"不经过路由表调用的画面的例外规约"章节）。
