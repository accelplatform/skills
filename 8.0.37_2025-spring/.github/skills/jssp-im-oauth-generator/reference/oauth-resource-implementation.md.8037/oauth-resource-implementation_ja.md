...
> 詳細は `.github/skills/jssp-page-generator/reference/api-account-context.md` を参照すること。
...
`application/json` ボディを受け取る場合の実装パターンは `.github/skills/jssp-page-generator/assets/post-json-api.md` を参照すること（OAuth 版でもボディ取得手順は同じ）。
...
- ログ出力時も同様（`.github/instructions/jssp-logging.instructions.md` の `MaskUtil` を活用）
...
- `.github/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()` の詳細
- `.github/skills/jssp-page-generator/reference/api-account-context.md` - 認証ユーザコンテキスト
- `.github/skills/jssp-page-generator/reference/argument-request.md` - request 引数
- `.github/instructions/jssp-error-handling.instructions.md` - エラーレスポンス形式・HTTP ステータス
- `.github/instructions/jssp-security.instructions.md` - SQL インジェクション・XSS 対策（OAuth API でも同様に適用）
...
- [ ] SQL アクセスがある場合は `DbParameter` でバインドしているか（文字列連結禁止、`.github/instructions/jssp-2way-sql.instructions.md` 参照）
