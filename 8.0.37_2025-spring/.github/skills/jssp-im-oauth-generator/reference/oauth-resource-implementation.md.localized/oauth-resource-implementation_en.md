...
- Same when outputting logs (use `MaskUtil` in `{{AGENT_RULES}}/jssp-logging.instructions.md`)
...
- `{{AGENT_RULES}}/jssp-error-handling.instructions.md` - Error response format / HTTP status
- `{{AGENT_RULES}}/jssp-security.instructions.md` - SQL injection / XSS countermeasures (applied similarly in OAuth API)
...
- [ ] When using SQL access, is `DbParameter` used for binding (string concatenation forbidden, see `{{AGENT_RULES}}/jssp-2way-sql.instructions.md`)?
