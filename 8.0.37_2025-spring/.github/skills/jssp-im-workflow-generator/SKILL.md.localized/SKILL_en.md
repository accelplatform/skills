...
│   └── schemas/
│       ├── listAuthorityPlugins.response.json
│       └── resolveAuthority.response.json
├── examples/
...
- For instructions not matching any of the above (e.g., "Users hired after 2026/10/01"), resolve via MCP `resolveAuthority`. See [mcp-spec/endpoints.md](mcp-spec/endpoints.md) for details.
...
2. **MCP `resolveAuthority`**: pass the natural-language description of the approver and pick `targetCode` from the result.
...
```jsonc
{
  ...
  "screens": {                                 // Individual screen path specification (auto-generated from screenBasePath if omitted)
    ...
    "tempSave": null,                          //   Temp save screen (pageType=1, null=default)
...
|-------|----------|---------|-------------|
| `matterEndProcess` | No | none | Case end processing script path (no extension). Plugin not registered if omitted |
| `matterEndProcessNoTransaction` | No | `false` | If `true`, uses the no-transaction extension point |

- With transaction: `jp.co.intra_mart.workflow.plugin.event.flow.matter.end.process`
- Without transaction: `jp.co.intra_mart.workflow.plugin.event.flow.matter.end.process.no.transaction`
...
Control IM-Workflow flow definition function settings with the `flowSettings` object in spec.
...
```jsonc
{
  "flowSettings": {
    ...
    "autoProcessLimitDay": null,    // Auto processing deadline days
    "autoProcessLimitType": null,   // Post-deadline action: 0=approve/1=deny/2=return (default when autoProcess=true: 0)
    "autoPress": false,             // Auto reminder (default: false)
    "autoPressLimitDay": null,      // Auto reminder deadline days
...
**Note:** The following settings are outside the scope of import XML and require manual configuration in the admin screen:
- Prevent applicant from self-approving (`applyUserApprovePreventFlag`)
- Case operation authorized persons (`handleUsers` output as empty array)
- Default organizations (`defaultOrgzs` output as empty array)
...
- This skill is exclusively for generating workflow **definition files**. For workflow integration **programs** (action processing, application screens, approval screens), use the `jssp-im-workflow-usage` skill.
...
| 1 | `tempSave` | Generate `tempsave/index` | Temporary save screen. Can be omitted with `false` |
| 2 | `applyTask` | **Omitted by default** (not output to XML) | Application (issued case) screen. Used only for periodic application patterns (monthly reports, fiscal-year goal setting, etc.) where a job auto-issues the case. Output only when a path is explicitly specified |
...
### How to Specify the `screens` Field
...
| **`undefined` / `null` / omitted** | Default behavior (see the table above) |
...
If you omit `screens` itself, the defaults shown in the table above are applied. Sample: [examples/straight.spec.json](examples/straight.spec.json).
...
| Skill | Purpose |
|-------|---------|
| **jssp-im-workflow-generator** (this skill) | Generate WF definition XML (contents/route/flow) |
| jssp-im-workflow-usage | Generate WF integration programs (.js/.html) |
