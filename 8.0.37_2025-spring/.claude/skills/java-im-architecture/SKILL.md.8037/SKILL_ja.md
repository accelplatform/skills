...
> 規約の要点（ガードレール）は `.claude/rules/java-architecture.md` にまとめている。本スキルはそこから参照される、完全なコードテンプレート・アンチパターン集・全レイヤー縦断実装例を提供する。
...
一般的なクラス・メソッド・変数の命名規則は `.claude/rules/java-naming.md` を参照。ここではレイヤー別の補足のみ記載する。
...
- 詳細は `.claude/rules/java-logging.md` を参照
...
ログレベル・出力パターン・例外タイプ別の判断は `.claude/rules/java-logging.md` を参照。
...
コードドキュメント（JavaDoc）の記述規約は `.claude/rules/java-javadoc.md` を参照。
...
- `.claude/rules/java-architecture.md` - 本スキルの規約要約（常時参照）
- `.claude/rules/java-naming.md` - 命名規則
- `.claude/rules/java-code-style.md` - コーディングスタイル
- `.claude/rules/java-javadoc.md` - JavaDoc規約
- `.claude/rules/java-logging.md` - ログ実装規約
- `.claude/rules/java-entity.md` - Entityクラス規約
