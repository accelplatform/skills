...
> 详情请参考 `.claude/skills/jssp-page-generator/reference/api-account-context.md`。
...
接收 `application/json` 正文的实现模式请参考 `.claude/skills/jssp-page-generator/assets/post-json-api.md`（OAuth 版的正文获取步骤也相同）。
...
- 日志输出时同样（活用 `.claude/rules/jssp-logging.md` 的 `MaskUtil`）
...
- `.claude/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()` 的详情
- `.claude/skills/jssp-page-generator/reference/api-account-context.md` - 认证用户上下文
- `.claude/skills/jssp-page-generator/reference/argument-request.md` - request 参数
- `.claude/rules/jssp-error-handling.md` - 错误响应格式・HTTP 状态
- `.claude/rules/jssp-security.md` - SQL 注入・XSS 对策（OAuth API 中同样适用）
...
- [ ] 涉及 SQL 访问时是否用 `DbParameter` 绑定（禁止字符串拼接，参考 `.claude/rules/jssp-2way-sql.md`）？
