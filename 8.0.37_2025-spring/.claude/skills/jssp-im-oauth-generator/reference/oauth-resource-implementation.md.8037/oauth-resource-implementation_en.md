...
> For details, see `.claude/skills/jssp-page-generator/reference/api-account-context.md`.
...
For implementation patterns when receiving `application/json` body, see `.claude/skills/jssp-page-generator/assets/post-json-api.md` (the body retrieval procedure is the same in the OAuth version).
...
- Same when outputting logs (use `MaskUtil` in `.claude/rules/jssp-logging.md`)
...
- `.claude/skills/jssp-page-generator/reference/api-web.md` - `Web.getHTTPResponse()` details
- `.claude/skills/jssp-page-generator/reference/api-account-context.md` - Authenticated user context
- `.claude/skills/jssp-page-generator/reference/argument-request.md` - request argument
- `.claude/rules/jssp-error-handling.md` - Error response format / HTTP status
- `.claude/rules/jssp-security.md` - SQL injection / XSS countermeasures (applied similarly in OAuth API)
...
- [ ] When using SQL access, is `DbParameter` used for binding (string concatenation forbidden, see `.claude/rules/jssp-2way-sql.md`)?
