...
| 認可 | `<authz>` で URI/action 指定（任意） | `<authz>` で `uri/action`（認可リソースで制御）を必ず指定する（`welcome-all` による認可スキップは禁止）。さらにアクセストークンの **scope** が `<client-resource>` の `<scope id>` と一致することが必須（AND 評価） |
...
2. **認可（authz）チェック**: `<client-resource>` の `<authz uri="service://..." action="execute" />` で指定する（**必須**）。認可リソース URI/action でロール／ユーザ単位に制御する。**認可リソースのインポート資材を別途用意する必要あり**（用意しないと常に 403）。`welcome-all` による認可スキップ（`<authz>` の省略やフォールバック）は禁止

両方を満たさない場合、プラットフォームがアプリケーションの `init` を呼ぶ前に 401 / 403 を返却する。
`<authz>` の書き方は `oauth-resources-config.md` の「`<authz>` の書き方」を参照。
