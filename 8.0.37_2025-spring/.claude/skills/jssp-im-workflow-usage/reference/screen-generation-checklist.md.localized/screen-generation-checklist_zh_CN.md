...
- [ ] 符合 `{{AGENT_RULES}}/jssp-presentation-page.md` 规范的验证函数组（`clearValidationError`、`showValidationError`、`createRequest`、`getValidationErrors`、`resetValidationError`、`validateCurrentStep`）按此顺序定义
...
- [ ] **条件必填**（根据特定输入值在必填/可选之间切换）字段的标签不以静态方式附加 `imds-required-label-required`，而是通过以下模式进行动态控制：
  ...
  - id 命名规约的详情请参阅 `{{AGENT_RULES}}/jssp-presentation-page.md`「id 属性的命名规约」
