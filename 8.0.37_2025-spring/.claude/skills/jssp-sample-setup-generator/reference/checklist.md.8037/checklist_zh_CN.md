...
`.claude/skills/jssp-tenant-setup-generator/reference/checklist.md`（替换方式参见 SKILL.md 的「参照时的替换」）。
...
- [ ] 遵循 `.claude/rules/jssp-code-style.md`（变量声明使用 `let`，不使用 `var`，字符串使用单引号）
