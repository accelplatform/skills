...
- **不创建路由配置（`.xml`）・路由授权**：Portlet 由门户功能（`b_m_portlet_info.path`）直接调用，不经过 `routing-jssp-config/` 下的路由表。因此不需要通过 `file-mapping` / `<authz uri="service://...">` 进行路由授权（访问控制改由 Portlet 自身的授权 `im-portal-portlet` / `im-portal-portlet-editmode` 来实现，详见 `.claude/skills/jssp-tenant-setup-generator/reference/portlet-import.md`）

需要说明的是，通过 IIFE 对绑定变量 `$data` 进行作用域隔离并非 Portlet 特有的差异，而是所有画面通用的标准实现（参见 `.claude/rules/jssp-presentation-page.md` 中的"绑定变量 `$data` 的作用域化（IIFE）"章节）。
