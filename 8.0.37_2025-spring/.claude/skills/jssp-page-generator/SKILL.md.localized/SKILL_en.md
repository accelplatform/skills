...
Refer to `{{AGENT_RULES}}/jssp-function-container.md` and `{{AGENT_RULES}}/jssp-presentation-page.md` to generate code.
...
- **Omitting JSDoc comments (`/** ... */`) or section delimiter comments (`// ===...===`) included in the asset.** Even if they appear verbose, the convention (`{{AGENT_RULES}}/jssp-function-container.md`) treats them as required. Keeping them clarifies the intent of each function inline, which makes subsequent reviews and modifications easier. Comments in assets must be copied as-is in principle. If changes are needed, only rewrite the content for the relevant feature; do not delete them.
