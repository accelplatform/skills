...
截至2026年，`.claude/rules` 目录下不存在专门规定异常处理的 Java 专用规约。`SubjectManager#removeSubject`/`removeSubjectGroup` 会抛出 `SubjectManagingException`（受检异常），而 `ResourceManager`/`PolicyManager`/`AuthorizationClient` 的主要 CRUD・权限确认方法以非受检异常为主（`InvalidResourceUriException` 等部分除外）。异常处理方式遵循 `reference/authz-api-reference.md` 中各方法定义及 `assets/authz-basic-usage.md` 中的模式。
