...
`.claude/rules` 下目前不存在规定异常处理方针的 Java 专用规约（截至2026年）。`AccountInfoManager` / `AccountPasswordAdapter` 的异常均为受检异常（详见后述），业务异常的包装方针应遵循 `assets/account-basic-usage.md` 中的模式。
...
7. 是否符合 `.claude/rules/java-naming.md` / `java-code-style.md` / `java-javadoc.md`
