...
No dedicated Java convention defining exception-handling policy exists under `.claude/rules` (as of 2026). Every `RoleInfoManager` method is designed to throw `AdminException` (a checked exception); follow the business-exception-wrapping pattern in `assets/role-basic-usage.md`.
